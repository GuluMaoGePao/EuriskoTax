...
- **主版本**：CHANGELOG 最新 **1.11.0**（阶段12：A 阶段核心功能补强（公式透明化 / 方案对比中心 / 计算核心纯函数化 / 常量版本化）+ C1 税制参数配置化（运维后台热改税率 + 版本化回滚 + 可选公告联动）+ 发布流程加固（`verify:pg` 演练门禁 + 两级回退 SOP）；上一版 1.10.0 = 悬浮税助手悬浮球重做）。1.11.0 已上线并打标签 `v1.11.0`
