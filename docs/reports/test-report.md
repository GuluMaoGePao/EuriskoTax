...
> 🟡 **历史快照 · 只作回查**：本文是一次性记录，文中数字（用例数 / 性能指标 / 交付清单）**已不是现状**。现状口径见 [CHANGELOG.md](../../CHANGELOG.md) 与 [文档中心「当前状态」](../README.md)。

**测试日期**: 2026-09-07（全量重跑基线）
**测试环境**: Node.js + Jest 29.7 + jest-environment-jsdom / Chromium 浏览器自动化
**项目版本**: 1.6.0（意见反馈落库 + 匿名计算埋点收尾后；测试套件自 v1.4.0 起无数量增减，203 保持全绿）
**测试命令**: `npm test`
...
- **2026-09-07 同步**：个人中心模块卡片由 6 张扩为 7 张（新增 `profile-card-feedback`「意见反馈」），`profile-page.test.js` 的 DOM fixture 增加对应卡片节点并断言渲染 7 卡；卡片数量不参与用例数，203/203 保持全绿。
...
1. **203 个单元测试全部通过**（6 套件，2026-09-07 复跑），无意外警告或错误
2. 计税核心函数性能达标（单次 < 3.1 μs），税助手高并发联想 < 200ms
3. 首页 + 个人中心两套件覆盖页面渲染、导航路由栈、档案校验、卡片导航（7 卡）等
4. 测试套件可作为回归基线：后续迭代运行 `npm test` 确保不引入回归（本次已同步"意见反馈"卡片的 fixture）
5. **链路门禁补充**：注册 → 邮箱验证码 → 邀请码 → 登录，以及反馈落库 / 管理员跟进 / 匿名埋点 / 聚合统计，已由 `npm run verify:local` 以真实后端覆盖（2026-09-07 扩至 20 项断言）；基于 Chromium 的逐 UI E2E 仍列待办。全量并行时 perf 时间断言偶发抖动（以 `--runInBand` 复核）
