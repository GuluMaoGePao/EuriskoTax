...
| L2610 | 重置开发测试账号 | `node server/scripts/reset-dev-user.js` | 原地重置本机测试账号密码（默认 dev@example.com / password，见 dev-account.local.json） |
...
| 行号 | 按钮文字 | 实现路径 | 关键行为 |
|------|---------|---------|---------|
| L3057 | 👤 项目登录账号复制 | `Set-Clipboard $devAccount.Email` | 本机测试账号（默认 dev@example.com） |
| L3059 | 🔑 项目登录密码复制 | `Set-Clipboard $devAccount.Password` | 本机测试账号密码（默认 password） |
| L3061 | 📋 一键复制邮箱+密码 | `Set-Clipboard "$($devAccount.Email) / $($devAccount.Password)"` | 排版好直接粘贴 |
