...
| v1.12.0 | `147c19e` | 阶段13 获客与转化 13A–13E（`Lead` 线索落库 + 前端触点与留资弹窗 + 管理台「线索」Tab + 漏斗埋点 + 一键结果分享图）+ 生产凭据轮换整改；门禁 **100/100**、单测 **20 套件 412 例**、线上指纹 **36 项** |
...
| v1.14.0 | `e999a11` | 阶段14 剩余项：首个高商业意图 SEO 落地页 `/seo/bonus-tax.html`（年终奖个税 —— 静态正文 + 同源口径速算器 + 六临界点跳档提示 + `robots.txt` / `sitemap.xml`）+ 门禁口径对账加固（`verify:local` 实跑项数快照与 `verify:release` 对账）；门禁 **152/152**、单测 **27 套件 558 例**、线上指纹 **37 项** |
| v1.15.0 | `905ef7a` | 阶段14 剩余项：第二个高商业意图 SEO 落地页 `/seo/salary-tax.html`（月薪个税 —— 累计预扣口径的静态预扣率表与 12 个月逐月示例表 + 同源速算器）；门禁 **156/156**、单测 **28 套件 571 例**、线上指纹 **37 项** |
| v1.17.0 | `13ea6b3` | 产品方向调整：取消计算页「参保城市」选择（回退阶段14 C2 端上部分，社保/公积金基数下限回全国兜底 7546）+ 城市改由留资收集（`Lead.city` 迁移 `20260913_add_lead_city` + 前后端四处接线 + 后台按城市搜索与 CSV 城市列）+ 三页「缴费比例」改可输入（含留空/非数字/越界回落 `normalizeRateInput`）+ 经营页「缴费基数 × 比例」接线修复 + 第三个 SEO 落地页 `/seo/annual-settlement.html`（汇算清缴）+ **发版口径守卫加固**（逐条校验 + 版本前缀约束，堵住同文件内旧口径漏检）+ **依赖漏洞清零**（根 `js-yaml` 3.15.1→3.15.2、server `qs` 6.15.3→6.16.0 via `overrides`，两侧 `npm audit` 均 0 漏洞）；门禁 **165/165**、单测 **28 套件 568 例**、线上指纹 **37 项**、`verify:pg` **165/165**（迁移 `20260913_add_lead_city` 生产等价验收通过） |

> 关于 v1.16.0（汇算清缴落地页）：其与 v1.17.0 的改动在同一工作批次完成、同属一个提交（`c653342`）且 `package.json` 一次性升到 `1.17.0`，从未作为独立发布状态进入 main，故**不单独打 `v1.16.0` 标签**（避免出现指向「非线上状态」的假锚点）；CHANGELOG `[1.16.0]` 条目作为文档沿用，其中「29 套件 592 例 / 门禁 162 项」是本批次中途口径，已被 v1.17.0 覆盖。
