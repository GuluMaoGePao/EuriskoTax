...
| v1.11.1 | `5f0a448` | 管理台「税率」Tab 保存链路修复（反馈位置不可见 / 版本号预填撞号 / 「载入出厂基线」清空表格）+ 缴费基数最低标准 4250 → 7546（与表单初始默认基数对齐，默认值即最低标准）+ `engine.test.js` 时间戳偶发假红修复；门禁 68/68、单测 17 套件 361 例。发布时本地 `:3000` 后端占用 Prisma 引擎 DLL，按脚本提示以 `-SkipVerifyGenerate` 复跑（schema 未变更） |
