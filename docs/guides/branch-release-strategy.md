...
| v1.12.0 | `147c19e` | 阶段13 获客与转化（13A 线索后端地基：`Lead` 模型 + 公开留资端点 + 管理端列表/统计/CSV；13B 前端转化触点：结果页情境引导 + 留资弹窗双通道 + 个人中心卡片；13C 管理台「线索」Tab：漏斗条 / 状态机 / 分配 / 导出；13D 一键结果分享图：`capture.js` 公共层 + 2 模板 + 二维码归因；13E 转化漏斗埋点：`FunnelEvent` 日聚合 + 公开上报端点）+ 生产凭据轮换整改（`ADMIN_TOKEN` / `JWT_SECRET` / `SMTP_PASS` / PostgreSQL 密码全量更换并逐项探测验证）；门禁 **100/100**、单测 **20 套件 412 例**、线上指纹 **36 项**。同版修复 `CHANGELOG.md` 中 `[1.11.1]` 标题被误删导致其正文挂在 1.12.0 标题下的归档缺陷。发布时本机无 Docker Desktop，`verify:pg` 按设计优雅跳过，改用「迁移 SQL 人工核对 + 双 schema 一致性检查」兜底 |
