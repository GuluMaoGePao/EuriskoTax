...
| v1.11.0 | `ca11756` | 阶段12：A 阶段核心功能补强（公式透明化 / 方案对比中心 / 计算核心纯函数化 / 常量版本化）+ C1 税制参数配置化（管理台热改税率 + 版本化回滚 + 可选公告联动）+ 发布流程加固（`verify:pg` 演练门禁 + 两级回退 SOP）；门禁 68/68、单测 17 套件 361 例 |
| v1.11.1 | `5f0a448` | 管理台「税率」Tab 保存链路修复（反馈位置不可见 / 版本号预填撞号 / 「载入出厂基线」清空表格）+ 缴费基数最低标准 4250 → 7546（与表单初始默认基数对齐，默认值即最低标准）+ 计算页/预算表样式打磨（表头不折行 / 金额等宽数字 / 顶栏底栏白底铺满视口 / 宽屏留白）+ `engine.test.js` 时间戳偶发假红修复；门禁 68/68、单测 17 套件 361 例。发布时本地 `:3000` 后端占用 Prisma 引擎 DLL，按脚本提示以 `-SkipVerifyGenerate` 复跑（schema 未变更） |
| v1.13.0 | `62d5e56` | 阶段14 变现与可信度（**14A/14B/14C `ProCode` 专业版兑换码**：`ProCode` 模型 + 迁移 + 后台事务化一码一用兑换（限时码续期 / 永久码 `plan_expires_at=null` / 已是永久 pro 拒绝且不消耗码）+ 用户端「版本与权益」弹窗自助兑换 + 管理台「兑换码」Tab（生成/计数/筛选/作废/导出，原 Tab 更名「邀请码」）；**14D/C2 城市社保参数库**：`CitySocialConfig` 版本化快照 + 公开只读端点（`since` 增量指纹）+ 管理台「社保基数」Tab（增删改 / 出厂基线 / 回滚另存）+ 三页「参保城市」下拉按城市校验基数下限（离线缓存重放，且在税率同步之后重新施加城市口径以保证时序））+ **发版口径工具化**（新增 `tools/ops/release-metrics.js` 口径定义单点 + `tests/docs-metrics.test.js` 文档口径守护 + `npm run verify:release` 一条命令核对版本五处与口径，`-- --write` 自动同步）；门禁 **146/146**（**口径修正**：此前记的 142 是「基线 100 + 阶段14 新增 35 + 阶段13 补强 7」的推算值，与实跑不符、少 4 项，本次按分段实测回填）、单测 **26 套件 545 例**、线上指纹 **37 项**。发布时本机 `:3000` 后端占用 Prisma 引擎 DLL，按脚本提示以 `-SkipVerifyGenerate` 复跑（schema 已变更，但 Prisma Client 生成时间晚于 schema 且已含两个新模型，跳过 generate 安全）；无 Docker Desktop，`verify:pg` 按设计优雅跳过 |
...
> 前置观念：**tag = 稳定锚点**。§5.3 表格里每一行都代表一次「线上指纹全绿的正式发布」（项数随版本递增，当前 **37 项**），
