...
| 改了 `schema.prisma`、或动过 `server/prisma/migrations/`，想确认「上线不会炸」 | **`npm run verify:pg`** | **生产等价演练**：用 Docker 起一个本地 PostgreSQL，按线上容器同序（`generate` → `migrate deploy` → 起服务）把同一套 225 项断言再跑一遍；`npm run verify:pg:fresh` = 先删数据卷（等价「全新库首次部署」）。需 Docker Desktop，未安装时优雅跳过（退出码 2，不是代码问题） |
| 单元测试 | **「运行全部测试 + 覆盖率」** / `npm test` | 42 套件 843 例（含版本号五处同步守护、文档口径守护） |
...
- 判定口径很简单：
  - **第一次用 / 刚拉取过代码 / 装过依赖** → 「第一次用：一键启动」（会自动 `npm install` + 重置本地测试账号，首次约 1–3 分钟）
  - **之前已成功启动过、依赖没变** → 「日常启动：快速启动」（秒起）

- 启动成功的标志：输出区出现 `服务器运行在 http://localhost:3000`，随后打印一批邀请码。
- 浏览器打开 `http://localhost:3000`，登录页在 localhost 下点「开发环境：填入本地测试账号」即可填入本机测试账号（默认 `dev@example.com / password`，可用仓库根 `dev-account.local.json` 覆盖成本机自己的账号）。
...
- **什么时候必须跑 `verify:pg`**：改了 `server/prisma/schema.prisma` 或 `server/prisma/migrations/` 之后。
  ...
  再跑一遍同样的 225 项断言；`npm run verify:pg:fresh` 会先删数据卷，等价「全新库首次部署」。
