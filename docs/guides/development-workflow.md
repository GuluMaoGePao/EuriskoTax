...
| 改了 `schema.prisma`、或动过 `server/prisma/migrations/`，想确认「上线不会炸」 | **`npm run verify:pg`** | **生产等价演练**：用 Docker 起一个本地 PostgreSQL，按线上容器同序（`generate` → `migrate deploy` → 起服务）把同一套 205 项断言再跑一遍；`npm run verify:pg:fresh` = 先删数据卷（等价「全新库首次部署」）。需 Docker Desktop，未安装时优雅跳过（退出码 2，不是代码问题） |
| 单元测试 | **「运行全部测试 + 覆盖率」** / `npm test` | 38 套件 739 例（含版本号五处同步守护、文档口径守护） |
...
- **什么时候必须跑 `verify:pg`**：改了 `server/prisma/schema.prisma` 或 `server/prisma/migrations/` 之后。
  ...
  再跑一遍同样的 205 项断言；`npm run verify:pg:fresh` 会先删数据卷，等价「全新库首次部署」。
