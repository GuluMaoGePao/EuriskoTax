...
**创建**: 2026-08-10
**更新**: 2026-09-11
**适用**: 主页、个人中心、税务助手、计算器四大模块的响应式布局维护与扩展
...
5. [计算器模块响应式规则（1 项）](#5-计算器模块响应式规则1-项)
6. [性能优化关联说明](#6-性能优化关联说明)
7. [维护检查清单](#7-维护检查清单)
8. [移动端验证方法](#8-移动端验证方法)
...
| 项目 | 值 |
|------|---|
| 选择器 | 各步骤底部按钮容器（上一步 / 重置 / 添加条目 / 下一步） |
| 位置 | [index.html#L2148](../../index.html#L2148) 等共 8 处 |
| 移动端 | `flex-col gap-3`，按钮 `w-full` 垂直堆叠 |
| 桌面端 | `md:flex-row md:items-center md:justify-between`，按钮 `md:w-auto` |
| 断点 | `@media (min-width: 768px)`（`md:`） |
