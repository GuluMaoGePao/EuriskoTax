...
*创建：2026-09-13 · 状态：⏳ 待开始（交付顺序：**先 15A 个税纵深 → 后 15B 企业税种**）*
...
| 编号 | 子任务 | 交付物 | 验收标准 |
|---|---|---|---|
| 15A-1 | 劳务报酬 / 稿酬 / 特许权使用费**预扣预缴**单算器 | 新页 + `withholding-quick.js` | 与内核公式逐点对拍；含「并入综合所得 vs 单独」对比 |
| 15A-2 | **股权激励个税**（股票期权 / 限制性股票 / 股票增值权 / 股权奖励） | 新页 + `equity-incentive-quick.js` | 单独计税 vs 并入综合所得对比；政策依据文号入注册表 |
| 15A-3 | **离职补偿金**免税额度测算（3 倍社平工资 + 12 年上限） | 新页 | 分档演示；超限部分并入综合所得口径正确 |
...
| 编号 | 子任务 | 说明 |
|---|---|---|
| 15D-1 | **税种注册表** `tax-registry.js` | 统一税率常量、生效期、政策依据、优惠条件，避免各页复制 |
