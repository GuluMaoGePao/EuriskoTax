...
**本阶段不做**：支付网关、微信小程序、迁移腾讯云（这三项均被 ICP 备案阻塞，单列阶段16）。
...
|---|---|---|
| 经营所得（个体户/个独/合伙） | **是（强）** | 经营所得汇算，还有扣除项可核对 → 免费咨询 |
| 汇算清缴 / 分类所得 | **是（强）** | 专项附加扣除是否填全 → 免费咨询 |
...
### 3.4 咨询情境（`lead-context.js`）—— 只展示能核实的信息

**问题**：早期实现把 `scene` 当成「当前测算」直接打印，但 `scene` 只是调用方硬编码的**入口标签**。
从个人中心进入时，弹窗会显示「当前测算：个人中心·财税服务」—— 个人中心根本没有测算，
等于在编造信息误导用户，也让顾问拿到一个无意义的字段。

**做法**：情境一律由 `src/js/lead/lead-context.js` 从可核实的数据反推：

| 进入方式 | 情境来源 | 用户看到 |
|---|---|---|
| 结果页（T1） | 读已渲染的结果 DOM，反推「类型 + 汇算结论 + 适用税率」 | 「参考您的测算：综合所得年度汇算 · 预计退税 · 适用税率 20%」 |
| 个人中心 / 其他无测算入口（T3） | 列出本机已保存的测算记录（`taxCalculationHistory`），由用户**主动选一条** | 下拉选择器（默认「暂不选择」） |
| 两者都没有 | —— | 整张情境卡隐藏（宁可不展示，也不编造） |

**合规红线**：金额节点只在本地用于判断「是否真的算完了」（hero 仍为占位符 `¥0` 时不出情境），
**任何金额都不进入 `scene`**。`scene` 里只有：测算类型 + 应退税/应补税/无需补退 + 适用税率 + 日期。

**为什么不直接上传历史记录的完整数据**：`scene` 上限 100 字符；且「不采集收入金额」是对外承诺
（弹窗同意文案与触点卡片脚注都写了这句话）。上传明细会立刻与这句承诺冲突。若后续确有业务需要，
应作为独立的「资料上传」能力单独设计并配单独的显式同意，而不是复用留资表单偷偷夹带。
...
2. **`scene` 情境快照** —— 顾问开场可说"我看到您在算个体户经营所得，扣除项那里……"，转化率远高于通用话术。
   写入前由 `lead-context.js` 归一化为**非金额摘要**（测算类型 + 汇算结论 + 适用税率），不含任何收入金额。
...
| `src/js/lead/lead-modal.js`（新增） | 弹窗组件：双通道 + 表单校验 + 提交 + 成功态；对外 `window.LeadModal.open({ source, type, scene })`（`scene` 仅在调用方已持有可信情境时使用，否则由 `type` 反推） |
| `src/js/lead/lead-context.js`（新增，13B+） | 咨询情境数据层：结果页真实摘要（不含金额）+ 本机历史记录摘要列表；对外 `window.LeadContext` |
| `src/js/lead/lead-touchpoints.js`（新增） | T1 结果页分流规则（白名单 + `BLOCKED_TYPES` 双保险）+ CTA 绑定；只传计算类型 `data-type`，不传死场景 |
...
- [x] 咨询情境只展示可核实的信息：结果页从已渲染结果反推（类型 + 汇算结论 + 适用税率，**不含金额**）；个人中心改为列出本机历史记录供用户主动选一条；两者都没有时整张情境卡隐藏（不再出现「当前测算：个人中心·财税服务」这种编造信息）
- [x] 弹窗文案「专业但像人说话」：标题给结论（「申报前先核对，该退的税别漏掉」），副标题点明具体查什么；不堆「前置核对 / 口径无误」这类公文词；信任点由竖排文字改为三枚胶囊标签（咨询免费 · 不采集收入金额 · 1 个工作日内回复）
- [x] 弹窗排版收敛：表单两段带序号圆标（1 联系方式 / 2 补充信息，选填段用灰标弱化）；底部操作区由「一行密集小字 + 右侧小按钮」改为「一句话同意 + 一行灰字说明 + 通栏主按钮」，末屏不再糊成一块
- [x] 触点 CTA 承诺的是**一次免费咨询**（「免费咨询」），不是「顾问替你完成申报」；单测守护禁用词（「免费核对一次」「首次免费」「把漏填项找出来」）
- [x] 弹窗只做「填信息」一件事：曾按同类产品做法加过「三步服务流程条」（`#lead-steps`）与「顾虑答疑」折叠块（`#lead-faq`），**已按要求整体下线** —— 用户不需要先读懂流程，填完信息由后台顾问联系反馈即可；单测反向守护（这两块被顺手加回来会变红）
- [x] 「提交后会有人联系」只留一句话：按钮下方「提交后由后台顾问与您联系反馈，无需其他操作」；成功态只说「已收到您的信息」+ 一句 JS 填入的补充，不再复述流程（提交完再读一段步骤清单，反而像没提交成功）
- [x] 顾问背书（`#lead-advisor`）由 `LEAD_CONFIG.advisorName / advisorTitle` 驱动：**未配置真实顾问信息就不显示** —— 真实背书可以少，但不能编
...
- [x] 取数直接读结果页已渲染 DOM（对计算模块零耦合）；hero 仍为占位符 `¥0` / `¥0.00` 时拒绝出图（一张写着 0 元的分享图比不出图更伤品牌）
- [x] 生成前预览确认弹窗 + 保存 PNG（手机端提示长按保存）；用户可在预览阶段放弃，不强制保存
- [x] 所有失败路径都有可见反馈：用页面内提示条（`showToast`）替代原生 `alert` —— 浏览器在用户多次触发后会静默屏蔽对话框，表现成「点击生成分享图没反应」；未识别的结果容器也不再静默 `return`，同时 `console.warn` 输出容器 id 与实际读到的值便于排查
- [x] 提示条固定在视口**顶部**并自动顺延顶部导航栏 / 离线横幅（底部提示落在结果区视线之外，容易被整条错过）；`z-index` 高于预览遮罩与页面横幅，配合下滑入场动效
- [x] 二维码归因（T4）：指向 `?source=share`，`lead-modal` 落地读取并记为留资 `source`；二维码库加载失败降级为域名文字，不阻断出图
- [x] 二维码 URL 支持外部配置：默认取 `window.location.origin`，线上部署可通过 `window.EuriskoTaxConfig.shareBaseUrl` 或 `ShareCard.setShareBaseUrl()` 覆盖，避免本地 `localhost:3000` 被生成到生产分享图里
...
- [x] 分享落地首屏引导（13D+）：`?source=share` 落地时展示「这是朋友分享给您的个税测算」横幅 + 「算出我的结果」CTA（锚点 `#home-start-card`）且可关闭；归因仍归 `lead-modal`、埋点仍归 `funnel-tracking`，本模块可整体下线而不影响闭环
- [x] 单测 `tests/share-card.test.js` 32 例（模板分流 / 与 index.html 的 selector 契约 / 合规 / 0 元占位符拦截 / URL 可配置 / 失败反馈可见性 / 归因与埋点闭环 / 公共层复用）；门禁新增 9 项静态断言
...
- **阶段15 税务计算能力扩展（多税种）**：个税纵深（劳务报酬/稿酬预扣 · 股权激励 · 离职补偿金 · 个人养老金 · 外籍优惠 · 汇算深度）+ 企业税种（增值税 · 企业所得税 · 附加税费 · 印花税）+ 社保薪酬 + 税种注册表与 SEO 落地页矩阵；纯前端能力，不依赖备案。见 [stage15-multi-tax-plan.md](./stage15-multi-tax-plan.md)。
- **阶段16 迁移与合规升级**（原阶段15，因新增阶段15 多税种扩展而顺延）：ICP 备案通过后 → 迁腾讯云国内节点（Zeabur 保留为预发）→ 官方支付 → 微信小程序 → B 端 API（B 端 API 另依赖阶段15 的 15B 企业税种）。见 [stage16-migration-and-compliance-plan.md](./stage16-migration-and-compliance-plan.md)。
- **与阶段12 的关系**：支付体系与 B 端 API 从阶段12 拆出后置到阶段16（被备案阻塞）；C2 移入阶段14。
...
| 2026-09-13 | 阶段重编号：新增阶段15 多税种计算能力扩展，原「阶段15 迁移与合规升级」顺延为阶段16 | 确认 |
