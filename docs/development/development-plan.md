...
*文档创建时间：2026-05-25*
*最后更新：2026-09-21（**v1.88.0 当前基线**：门禁 **259/259**、单测 **98 套件 1872 例**、线上指纹 **37 项** ——本版「阶段19-6b 我的页：权益进度条 + 资产概览 + 功能按场景分组（§3.6 ①②③）」：权益进度条 `#profile-benefits-card` 把"我是什么版本"换成"我用到了哪一步" —— 三格阶梯点亮到当前档，下面两行是真数（已用 N/M 套方案、云同步开没开 + 上次同步时间）；**进度条只表达档位在阶梯上的位置**，不表达"距离专业版还差百分之几"（基础版到专业版之间没有分母，任何百分比都是编出来的），取不到的整行不出现（不补 0 —— "已存 0 套" 会被读成"你一套都没存"），升级入口**未新增**（就是横幅那颗 `#profile-nav-upgrade` 搬了下来，全局仍只有 2 处）；资产概览四格全换 —— 原「计算次数 / 档案数量 / 历史记录 / 本月提醒」里两格**永远同一个数**（都读 `taxCalculationHistory.length`）、一格只有 0/1、一格只由当前月份决定，现为测算次数（本月 N 次）/ 已存方案 / 覆盖税种 / 上次测算，取不到一律「—」，最近一次按**时间**取最大（云同步合并后顺序不保证），「本月提醒」改挂税务日历卡的 badge；功能卡按场景分三组（我的数据 / 我的税务 / 服务与支持）。上一版 **v1.87.0**（门禁 **259/259**、单测 **97 套件 1855 例**、线上指纹 **37 项**）——「阶段19-6a 档案可查看 / 修改 + 省钱卡（§3.8 ④）」：我的 → 税务档案顶部新增「我的情况」卡 —— 19-5 的引导卡是**单向**的，一次只问一项、补完没有出口，改个城市都无处下手，而一份只有「写」没有「读改」的档案，用户只会觉得它在偷自己的数据；五项一次列完并可直接改，写回 `tax-profile.js` 的那份存储（不另起 key），旧表单改名为「默认参数（预填数值）」—— 一个是"我是谁"、一个是"默认填什么数"，不能混在一张卡里；引导卡底部补出口「在『我的 → 税务档案』里查看 / 修改」。省钱卡 `#quick-saving-nudge` 只说**算得出来**的优化空间：年终奖择优是真算的两种口径差额（没填「全年其他应纳税所得额」就提示去填，不给编出来的差额）；专项附加扣除漏填只说漏了哪几项、**不编金额**（几个子女、怎么分摊都不知道，硬算出来的 ¥1,200 是假的）；两种口径一样就不出卡。顺手修：`profileAfterPatch` 写死重画速算器那张卡，导致完整测算结果步里点档案选项毫无反应。再上一版 **v1.86.0**（门禁 **259/259**、单测 **96 套件 1841 例**、线上指纹 **37 项**）——「阶段19-5b 与上次对比 + 完整测算接入档案引导（留存机制 §3.8 ③）」：同工具算到第二次，结果页出现对比卡（上次 ¥X · 这次 ¥Y · 少缴 / 多缴 ¥Z，取自 `taxCalculationHistory` —— 两次是同一口径同一个 compute，差额是真算出来的）；**比不出来就不显示**（没有上次 / 上次是别的工具 / 取不到税额），摆一张比不出差额的对比卡，用户会以为这里有过结论，那比没有更糟。没做成 plan 原文的「存为方案 A」是刻意的：方案库那张对比表只覆盖综合所得口径，速算器存进去后缺的指标会被 `fmtValue` 补成 **¥0.00** —— 那是假数据，宁可少一个入口。deep 结果步新增 `#dw-profile-nudge`，复用速算器那一份档案引导渲染（不抄第二份），卡内元素一律改成 class + 容器作用域定位（同页两张卡用全局 id 会互相覆盖 —— scenario-ui.js 早年被这个坑过）。
*对应项目版本：v1.78.0*
