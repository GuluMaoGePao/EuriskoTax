...
*文档创建时间：2026-05-25*
*最后更新：2026-09-23（**v1.103.0 当前基线**：门禁 **259/259**、单测 **112 套件 2092 例**、线上指纹 **37 项**）——本版「政策时效与版本钉上结果页（19-12）」：§2.5.2 断点 B3 说的「现成资产在睡大觉」半真半假 —— `tax-registry.js` 里 statusOf / expiringWithin 数据层全有，速算器页也渲染了一枚徽标，但 **21 个完整测算一枚都没有**（财务与代账的主力入口，他们最怕的就是「政策过期没」）：同一个 policyKey，两个入口两套可信度口径 —— 这不是「少一个功能」，是同一个产品在两个入口上口径分裂。改法：徽标只有一份（速算器出、deep 只管用），简明视图下也显示（**可信标记不是参数**，藏起来等于让用户不知道这次算里有没有已经过期的政策）；B4 顺手补上 —— 结果与 PDF 导出都标「政策版本」（tax-registry.VERSION，渲染层**不自带数字**：自带就一定会在改口径时忘了同步，印着一个对不上的版本号比不标更糟，因为它看起来像证据）。测试抓出真数据洞：6 个 spec 根本没挂 policyKey（不报错、只是永远没徽标），补上 5 个；**classification（分类所得）刻意不挂** —— 政策库里没有这个条目，为凑齐徽标编一条政策（文号与有效期会印进导出的「政策依据」）比不挂更糟：不挂只是没徽标，挂错是给用户一个**错的有效期**，那是在骗人；缺口进测试白名单（不许默默变多）。基线抓出真回归：徽标挤在标题行里，375px 下把「月薪个税」压成一字一行 —— 改为独占一行。守护 `tests/policy-freshness.test.js` **14 条**（数据无洞 / 两类入口同一枚 / 简明下也显示 / 三种时效措辞 / registry 拿不到就静默不出 / 版本来自 registry 不给就不标）。基线重拍 quick-375/1280 与 deep-result-375/1280 共 8 张（总量 28 不变）；真机已验：deep 结果卡与速算器页的徽标文本逐字一致。
