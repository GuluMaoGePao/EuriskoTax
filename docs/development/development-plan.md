...
*文档创建时间：2026-05-25*
*最后更新：2026-09-23（**v1.102.0 当前基线**：门禁 **259/259**、单测 **111 套件 2078 例**、线上指纹 **37 项**）——本版「视图信号写进线索：谁真的想要细节，顾问该先看谁（19-7b ②）」：两条天差地别的线索（完整视图 + 调过进阶参数的财务 vs 简明视图只填了月薪的路人）在线索表里长得一模一样 ——用户在界面上早就交代了自己是谁（**要不要看更多参数是一次明确的自我介绍**），这信息以前白扔了，而且零采集成本：不用多问一句、不含任何金额与填报内容，只记「使用深度」。四个落点：① 记录放 **mode-pref.js**（折叠块是它定义的），两个渲染器各自发明一个存储键就是阶段18 那个病；② **只有简明视图下的展开才计**（完整视图下它本来就是展开的，点一下不代表在找参数，算进去会把两类人混成同一个数）、且只有「更多参数」这个块算，推导链那些 details 不算；③ 监听挂在 document 的**捕获阶段**（toggle 不冒泡，而速算器与向导都是整块 innerHTML 重画，逐个 details 绑定会被下次重渲染冲掉）；④ **必须落库才有用**（顾问看的是线索表与导出的 Excel）—— 线索表加 `view_mode` / `advanced_touched` 两列 + 迁移 + CSV 两列：`view_mode` **允许为空**（老客户端没上报就是没信号，不能回落成 simple 充数，否则顾问会把「没采集到」当成「用户选了简明」），合并时 `advanced_touched` 取 **max**（它是事实不是状态，不能被后一次「这次没展开」抹掉）。刻意**不并进** `current()` 的情境文本 —— 那段话是给用户看的，混进行为标记等于当面给用户打分；取不到信号返回 null（没信号就说没信号）。守护 `tests/lead-view-signal.test.js` **14 条** + `tests/leads.test.js` **2 条**，含跨 4 个文件的**字段名对拍**（任一侧改名都不报错，只是顾问从此看不到信号 —— 静默失效里最难发现的一类）。真机已验：简明下展开计数归因、完整下不计数、**拦下 submitLead 的 payload，字段真的带上了**；视觉基线 28 张零差异。
