...
*文档创建时间：2026-05-25*
*最后更新：2026-09-23（**v1.101.0 当前基线**：门禁 **259/259**、单测 **110 套件 2062 例**、线上指纹 **37 项**）——本版「字段级分级铺到全部完整测算（19-7b ①）」：19-7a 只给 6 个高频完整测算标了**步**级 advanced，其余 15 个 spec 一个分级都没有 —— 默认简明的人点进去看到的还是一整屏参数，双视图对它们等于没上线。判定标准只有一条：**不填也能算出结果**（有 `default` 兜底）、填了更准的才标 advanced；据此三条硬边界 —— 主输入不标（藏起来不叫简洁，叫算不出来）、决定「这次算什么」的分支不标（藏了等于悄悄换掉工具语义）、整步都是可选的标**步级**而步内只剩一部分可选标**字段级**，且两者**不叠加**（既在 advanced 步里又自带 level 会折出两层折叠，用户要连点两次）。分级收在注册表末尾 `ADVANCED_FIELDS` / `ADVANCED_STEPS` 两张清单里，**不散到 300 个字段定义上逐个打 `level`**：一份清单能一眼看全「哪些参数算进阶」，散进字段定义就再没人核对得过来 —— 这与「按名字认人的表，每加一种形态就漏一批」是同一个病、同一剂药（按 id 认，不按位置认）。顺手修一处真 bug：`private-pension-deep` 的 `growthMultiple` 写了两个 `step`（`'pension'` 与 `0.1`），后者把前者冲掉，字段因此不属于任何一步 —— 界面上**永远看不到它**，只能按 default 1.5 参与计算（能算、但改不了）。守护 `tests/field-level.test.js` 9 条（分级铺到了全部 spec / 标了 advanced 的字段必须有 default / 每个 spec 至少留一个 basic 字段 / 步级与字段级不叠加 / 字段的归属步必须存在 / 速算器一个都没标 / 简明下控件仍在 DOM 且默认收起、完整下默认展开）。真机已验（简明下折叠块默认收起、完整下默认展开、切换视图不丢值、修好的 `growthMultiple` 可见；视觉基线补 `deep-step1-advanced` 一张 target 共 4 张，基线 24 → 28 张）。上一版 **v1.100.0**（门禁 **259/259**、单测 **109 套件 2053 例**、线上指纹 **37 项**）——「省钱卡接进 21 个完整测算（清偿 19-4 顺延：省钱卡）」：19-6a（v1.87.0）那张省钱卡只挂在速算器结果页，21 个完整测算的结果步算完就到免责声明 —— 而**真正会漏填专项附加扣除的恰恰是这些按年填全的测算**。适用范围原先写的是「tool.group 必须等于 salary」（速算器才有 group，deep spec 一个都没有，按这个判它们永远进不来），改成**按 spec 自己的 fields 判**：有 label 含「专项附加扣除」的字段 = 这次测算吃它才可能漏；不写 id 白名单（加一个 spec 就要回来改一次名单，那是 19-5b 那条债的同一个形状）。这么判顺带**修正一处既有误导**：bonus-tax 原先因 group 会被报「档案里有扣除这次没算进去」，可**年终奖单独计税不吃专项附加扣除**，那是在催用户补一个对他这笔奖金无效的数；special-deduction（核定额度那个）同样自动排除 —— 它的字段是「子女教育（个数）」这类逐项，它自己就是在算这个。出口在 deep 里是「回去补上」而不是「跳去另一个工具」（后者等于让用户把分步填完的一整份丢掉重来）：tip 上带两个出口，按「那一步此刻找得到吗」挑 —— 找得到就回那一步（走的是与「上一步」同一套 stepIndex + saveDraft + render，不为这张卡新开导航路径），找不到（简明视图把 advanced 步合并了）就退回「核定能扣多少」。plan §3.5 列的第二个场景**年终奖择优在 deep 侧刻意不做**：bonus-tax-deep 本身就在做择优，再告一遍是噪音，且它算的是含最优拆分的口径，与速算器那份「两种计税方式差额」不是同一个数 —— 完整测算侧只有「专项附加扣除漏填」一条，一条真的好过两条里有一条是编的。守护 `tests/saving-nudge-deep.test.js` 11 条（含「卡里一个 ¥ 都没有」与「速算器年终奖不再被误报」两条）。⚠️ 真机未验。上一版 **v1.99.0**（门禁 **259/259**、单测 **108 套件 2042 例**、线上指纹 **37 项**）——「结果页『各项数额』条（清偿 19-4 顺延：结果可视化）」：plan §3.5 那句「引入 Chart.js（**已在依赖里**，`final-report.js` 已用）」动手前查了是**错的** —— 全仓 0 处引用，为一张横条引一个 canvas 库的代价是新增 CDN 依赖 + 深色要重画 + 24 张基线全重拍 + jsdom 测不到，所以走**纯 CSS 横条**（`div` + `width%`，颜色用既有令牌，深色/响应式/降级自动跟随）。第二处偏离：plan 要的是「收入构成 → 扣除 → 应税 → 税额」四段瀑布，但瀑布要按**行名认语义**（哪段是减项、哪段是基数），41 个工具没法通用给出，硬配一份映射就是 19-5b 那条债的由来（一份映射迟早与界面上正在显示的数漂移）—— 所以这张条**不猜语义**，只画各项金额的相对大小（最长一项 = 100%），主结果用品牌色标出、其余中性灰，说明里明写「各项之间不构成加减关系」。三条纪律：凑不出两根**整块不出现**（一根条没有"比"的对象）；负数与 0 不上条、**不拿 0 充一根条**（唯一例外是主结果 —— 「这个季度不用交税」本身是结论，滤掉就是把结论藏起来，实测增值税小规模默认入参主结果正是 0）；**只认金额行**（把「实际税负率 13%」和「销项税额 ¥3,000」画在同一根尺子上比大小，是制造一个不存在的比较）。落点：新增 `src/js/ui/result-bars.js`（纯函数 `buildBars` + `html`），样式在 `ui-redesign.css` §4.5；**一份实现两处挂载**（速算器结果区 + 21 个完整测算结果步，两边结果对象本就是 `primary + rows` 同一形状），取的都是**界面上正在显示的那一份**（`out` / `view`）不会再算一遍；位置在主结果 Hero 之后、明细行之前；最多 5 根且截断要写清（不静默）；默认展开可收起（折叠 ≠ 藏起来）；模块没加载时 `console.warn` 明说。守护 `tests/result-bars.test.js` 20 条 + `toolbox-ui.test.js` / `deep-wizard.test.js` 各一条钉挂点位置。⚠️ 真机未验（playwright 会话仍起不来）。上一版 **v1.98.0**（门禁 **259/259**、单测 **107 套件 2020 例**、线上指纹 **37 项**）——「方案库口径补齐：任一工具都能存方案（清偿 19-5b 遗留）」：方案对比那张表的指标原先写死是综合所得那六项（税前 / 税后 / 税额 / 税负率 / 月均 / 年终奖口径），别的工具存进去，缺的几项会被 `fmtValue` 补成 **¥0.00** —— 假数据比没有更糟，所以 19-5b 交付时**没给速算器接方案库**，速算器侧改成跟同工具的上一次测算比。现在方案按**工具自己的口径**存指标：`scenario-store.buildMetrics(out)` 把结果对象（主结果 + 明细行，20 个速算器与 21 个 deep 共用同一套形状）摊成 `[{label,kind,value}]`；对比时按 label 取**交集**，同一工具天然可比，跨工具交集为空就**各列各的**（不硬凑一张"看起来能比"的表）；取不到画 `-` 而不是补 0；新指标**不标「最优」**（哪头更好猜不出来：应退税额越大越好、应纳增值税越小越好，猜错就是把错的标成最优），`best` 只对老口径那六项生效；老方案（只有 `summary`）由 `metricsOf()` 摊回老六项，照样显示、照样可比，**不写迁移脚本**。入口：速算器行动条加小字链接「存为方案（加入对比）」（不占第五颗按钮位）；完整测算结果步的方案卡不再只给声明了 `toCalcInput` 的 spec，21 个都能存，「生成年终奖方案」只在综合所得出现（摆一颗点了必然报错的按钮是拿 UI 骗人）。出口：方案库每行加「载入」 —— 把存的那份输入带回对应测算，分发**不新写 if**（`EuriskoToolbox.openTool` 内部已按注册表分派，18-2 修的就是这个）。首页差额改从两套共有的指标里挑**税额类**那一行（`diffRowOf`），对不上就不说差额。坑两条：`Number(null)` 是 0（空值必须先判空，否则"没算出来"被记成"算出来是零"，`buildMetrics` 与 `normalizeMetric` 两处都踩了）、`bestCellIndex` 方向未声明时返回 0（现在非 min/max 直接 -1）。`tests/scenario-metrics.test.js` 20 条；端到端那条断言的取数落点从 `summary.netIncome` 换成 `metrics[0].value`（守护的仍是"存进去的数 = 界面上显示的数"）。上一版 **v1.97.0**（门禁 **259/259**、单测 **106 套件 1999 例**、线上指纹 **37 项**）——「阶段19-2 遗留清偿④：首页『我的方案与台账』」：原「最近计算」是一次性流水 —— 算完往历史里一扔，看不出**存了几套方案、这个月办到哪一步**；且方案对比表原先**只挂在结果页**，存了 2 套方案下次想看必须先重算一遍，否则躺在 localStorage 里没有任何入口能打开。现在：首页原「最近计算」卡升级为「我的方案与台账」—— 卡内新增**方案对比**段（`已存 2/2 套（专业版 10 套） · 并入工资 / 单独计税 差 ¥2,180`）与**台账**段（`2026 年 9 月 · 已算 2 条 · 1 条未申报`，整格同一主体才带主体名），流水降为下方一小段（加「最近算过」小标题区分，**不撤** —— 它是"接着上次算"的入口，撤了就是丢功能）；新增 `scenario-ui.js` 的 `openLibrary()` 方案库弹窗 —— 与结果页**共用同一张对比表**（`headHtml / tableBodyHtml / bindDeletes` 抽出共用，不写第二套），台账出口 = 既有 `openLedger()`。四条边界（`tests/home-plans.test.js` 18 条）：① **零感知**（没存方案/没有台账行两段整段不出现，新客看到的还是原来那张卡）；② **出口必须真能打开**（段里不出现点了没反应的东西）；③ **不编数**（差额取不到就不说；税额相同说「税额相同」而不是「差 ¥0」；主体名只在整格每一行都归同一主体时才报；台账说**最近一个期间**不是硬编码"本月"；「未申报」= 办到哪一步，不是逾期）；④ **不新增升级入口**（上限照实说但不挂按钮，全局仍只有两处）。顺带：`home-ui.js` 首次引入 `esc()`（方案名与主体名是用户自己起的，拼 innerHTML 前必须转义），千分位自己写（`toLocaleString` 随 ICU 变，断言会飘）。真机已验（空态两段不出现 / 播种后两段齐全 / 看对比弹窗删一套表里剩一套 / 我的台账弹窗打开 / 浅深正常）。上一版 **v1.96.0**（门禁 **259/259**、单测 **105 套件 1981 例**、线上指纹 **37 项**）——「阶段19-2 遗留清偿：身份卡『设为默认视角』」：plan §3.3.4 写死「身份卡的作用是设置默认视图，**不是筛选工具**」，但落地成了 `openScenario()` —— 跳工具页后**只渲染那一个推荐组**，其余 36 个入口当场消失（那是筛选，还违反 §3.9「41 个永远都能进」的硬边界）。现在拆成两个动作：**点卡主体 = 设为默认视角**（就地：新增 `src/js/ui/identity-pref.js` 记住身份、卡上亮「当前默认」徽标、反馈行说明生效了什么并给「取消默认」退路 —— 设默认不跳页不筛选，屏幕上没变化用户就不知道生效了什么）；**「看这类工具 ›」= 跳工具页**（推荐组置顶展开，其余分组照常渲染，完整测算组不再被收起，chip 文案「按身份筛选」→「按身份推荐」—— 名字说错了，后面的人就会照着名字把它做成筛选）。三条边界：① **身份不写计税档案**（档案 identity 是计税口径 3 个值，身份卡 6 张，值域对不上 —— 选了「HR」往档案写什么？两个真源双向钉死）；② **不推翻用户的显式选择**（身份带出 plan 表给的默认密度：老板/财务→完整、上班族/自由职业/HR→简明；但用户亲手切过视图后就只听用户的 —— `mode-pref` 新增 `isExplicit` + `applyDefault()`；「取消默认」只清身份不动密度）；③ **plan 没给答案的不编**（「高管/股东」未登记默认密度，选它只记标签，反馈行不显示那句"当前视图"）。`tests/identity-pref.test.js` 18 条；`toolbox-ui.test.js` 原「按身份筛选」一条断言的正是要改掉的旧行为，改为钉可达性。真机已验（设默认不跳页 / 看这类工具 25 个入口全在 / 用户先切简明再选财务 → 密度仍简明）。上一版 **v1.95.0**（门禁 **259/259**、单测 **104 套件 1963 例**、线上指纹 **37 项**）——「阶段19-2 遗留清偿：首页『漏填提醒』卡」：19-2 当初欠下的这条债（plan §3.3 ④），判定与出口的依赖到 19-6a 才齐。判定**不自己写**：漏了哪几项 = `tax-profile.js` 的 `completeness()`，出不出卡 = `shouldNudge()`（结果页点过「暂不」，首页同样不再提）；两条不出卡的规矩：一项都没填的新客不显示（那是结果页引导卡的活）、全填完不显示（没有"还能再补"还占位就是噪音）。**金额一律不编**（漏项能省多少取决于几个子女、怎么分摊，守护测试钉死模型里不存在金额字段、正文不出现 ¥）。出口**开速算器而不是跳「我的」**：那张编辑卡要登录，而免登录可直接算是主线；算一遍对应工具既能把值补上（`absorb` 吸回档案），也不必再写第二套补档案表单 —— 映射：城市/社保→社保公积金、专项附加扣除→专项附加扣除、年终奖→年终奖、身份→滚到首页身份卡组，**守护测试要求档案每一项都有去处**（往档案加一项忘了给出路测试直接红）。三个时机都重画（进首页 / 算完保存 / 订阅档案变更事件），样式零新色（漏项 chip 复用 `.profile-chip` 配色、说明行复用 `.whoami-hint`、按钮走 `btn-primary` 令牌）。`tests/home-missing.test.js` 15 条；真机已验（playwright-cli：出卡、CTA 开对工具、补满消失、新客不出卡、浅深两档正常）。上一版 **v1.94.0**（门禁 **259/259**、单测 **103 套件 1947 例**、线上指纹 **37 项**）——「阶段19-11 效率层 E4 批量」：前三项解决的是"同一件事**每月**做一遍"，而 HR 手里从来不是一条记录，是**一张表**（每人一行、一次几十上百行）—— 这份表原先在本产品里没有着落，只能一个人一个人地填，填到第 30 个时前面 29 个已经没人记得有没有填错。新增 `src/js/ui/batch-ui.js`：粘贴 → 认列 → 预览 → 计算 → 导出（CSV 下载 / TSV 复制贴回 Excel），页面 `profile-batch-page` 由个人中心「工作台」组第四张卡进入，**不动主 Tab**（与 5 个既有子页同一套路）。三条边界：① **批量不另起一套算法** —— 每一行都走 `tool.compute(values)`，与速算器逐个手填同一个入口，防两套口径的闸是"批量 N 行 = 单行逐个算，逐项相等"，且对**每一个**速算器对拍（不是挑两个代表）；② **数据不出本机** —— 没上传、没落库，批量结果**不写计算历史**（200 行灌进历史，历史就不再是"我算过什么"），这句话写在页面顶部而不是脚注；③ **免费 5 行/次，超限拒绝整批而不是截前 5 行** —— 半份工资表看起来跟完整的一份一模一样，发出去就是事故。只做速算器：完整测算（deep）是分步向导、字段之间有依赖，按列喂进去会缺字段，那不是"算不准"而是丢了"填到哪一步"这个语境。列识别有退路也有说法（表头带单位/空格/括号都认；一个字认不出才退顺序映射并明说；未识别的姓名列**原样带出**，对不上人的表没法用）。脏数据分两种：空着 = 这项没有（0，照算）；写了却读不懂（"面议"）= 标出来而不是算成 0（算成 0 就成了"这个人不用交税"）。**不做**：多主体同场景批量（plan 第二小步）、跨设备同步/保存批量结果（数据不出本机是卖点）、deep 批量。上一版 **v1.93.0**（门禁 **259/259**、单测 **102 套件 1927 例**、线上指纹 **37 项**）——「阶段19-10 效率层 E3 台账」：重复报税的真实形态是一件事**每月做一遍**（同一工具、同一口径、同一主体），历史能留住每条流水，却答不了"这个月还有哪件事没办"、也说不了"跟上个月一样的参数再来一遍"。台账补的就是**期间 × 主体 × 进度**这一层。**做成第二份存储是最容易走的那条路，这一版刻意没走**：历史是本体，`ledger-store.js` 是它之上的索引，索引里只存历史自身查不出来的三样东西 —— ① 期间默认从记录自己的 `date` 推（它本来就写着"什么时候算的"，再抄一份两边迟早不一致：改了期间后索引说这个月、date 说上个月，听谁的？），**只有用户显式改过**才写覆盖值；② 主体只能存索引（`attach` 默认取当前主体）；③ 状态（已算 / 已导出 / 已申报）是"办到哪一步"，不是测算的一部分。这么分工的收益碰上线才知道：索引读坏了期间能从 `date` 重建、主体与状态丢了只是少一层标注，**历史一条不少、一个数都没改**。写入点只有两处（速算器与 `tax-calculator.js` 的 `saveToHistory`）—— **保存即入账**，这是**有意偏离 plan 原文**的一处：原话是"结果页行动条改为「存为模板 · 加入台账」"，但上一版刚把行动条上那颗只负责回工具页的第四颗撤掉，再加回来就是自打脸；归档跟着保存发生，要改期间 / 状态 / 归属去台账弹窗改。`data-management.deleteHistoryRecord` 也连了线：历史删了台账那行跟着走 —— 留下来就是一排"点进去什么都没有"的幽灵行。界面 = 工作台组第三张卡「我的台账」→ 台账弹窗（按月分组、每行的期间与状态就地改、**有上月账的行才给「复制上月」**），权益只影响**看几个月**（免费近 3 个月、专业版不限），更老的一条都不删 —— 门槛设在视野上而不是数据上，因为删掉的数据用户没法自己变回来。不做：往申报台再走一步（申报表 / 回执 / 对接税局是另一个产品，`status` 只是用户给自己看的标记，系统不根据它做任何事也不催任何人）、跨设备同步台账（要后端）、**首页「台账待办」小卡**（plan 原文"台账未申报项 × 税务日历"那一环留到下一版 —— 它得先答清"什么算待办"，现在做等于替用户判定，会把记账变成瞎催）。上一版 **v1.92.0**（门禁 **259/259**、单测 **101 套件 1913 例**、线上指纹 **37 项**）——「入口清理」：**同一份数据不摆两处，行动条不放复制品** —— ① 撤掉首页「最近使用」卡：它与工具页第一组「最近使用」读的是**同一份 localStorage**，等于同一份数据在两个地方各显示一遍，而首页紧接着还有「最近计算」卡，连着三张都在讲"你最近算过的"；回访入口保留在工具页那一组（要找下一个工具本来就要进工具页），**「清空」跟着搬到组内** `#toolbox-recent-clear` —— 挂在组内而不是组头上，因为组头本身是颗 `<button>`，往里面塞按钮会变成嵌套 button、键盘与读屏都会错乱；不补回来的话列表只能靠新记录挤掉旧的，那不是收敛入口，是丢功能。② 结果行动条第 4 位不再放纯导航：原先"没有同名完整测算"时这一位是「换个工具」，点下去 `showPage(TOOLS_PAGE)`，与页面左上角那颗返回箭头**同一个目的地**、同屏出现两次；现在这一位只承认"再往前一步"的内容（有同名完整测算 → 「按年填全的完整版」／有相关工具 → 「算完还能干什么」跳到下文推荐区／都没有 → **这一位空着**，宁可三按钮是真的三个出口，也不要第四颗来复制左上角）。③ 删掉一条假接线：`home-ui.js` 里的 `refreshRecentTools()` **全仓没有定义**，`typeof` 判空让它永远安静地跳过，那条礼貌的 `try/catch` 掩盖了一个从不执行的分支。动刀前先把"是不是真重复"钉清楚：顶部 Tab 与底部 Tab 是**同一状态的两种投影**（手机拇指区 vs 桌面导航在上）；工具页 4 张静态 mode card 与动态追加的 21 个 spec 入口由 `renderDeepEntries()` 按 `t.id + '-mode-card'` 排重、两边不重叠；升级入口仍只有顶栏 pill 与权益进度条两处 —— 这些看着像重复其实不是，一律没动。上一版 **v1.91.0**（门禁 **259/259**、单测 **101 套件 1912 例**、线上指纹 **37 项**）——「阶段19-9 效率层 E1 主体 + E2 参数模板」：新增 `src/js/data/entity-store.js` + `src/js/data/template-store.js` + `src/js/ui/entity-ui.js` —— 19-8 的参数记忆解决的是「**上一次**填的值」，但重复报税的真实形态是「**同一批人对好几份不同的参数**」（自己 + 代账的几家客户 / 公司 + 个体户），那些值不是「上一次」而是「**某一份**」，所以记忆之外还需要两件事：给这批参数**起名字存下来**（模板）与给它们**归个类**（主体）。主体刻意钉死成**只有名字 + 备注的标签** —— 不参与计算、不注入任何字段、没建主体时顶栏那块 DOM 一个字都不留；一旦给它带上属性（税号 / 征收方式），它就必须参与默认值推导，「为什么默认这样算」就有了第二个答案。两条硬边界：① 模板**存入与取出都按注册表字段表白名单过滤**（跨版本字段会变，留着未知键就是给未来的自己埋雷）；② 模板记的是**此刻表单全量**，含当前没显示出来的条件字段（显示层按口径收起，存储层不收）。接入三处：顶栏主体切换器（无主体整块隐藏）/ 工具页「从模板填充」折叠条（没模板整条不出现，用原生 `<details>` 自带键盘）/ 结果行「存为模板」（**失败必须把原因说出来**，静默失败等于让用户以为存了、下次来看不见）。踩坑三条：① **套用模板不能整表单重建** —— 原 `build()` 每次按 seed 重画，用户改个 select 就把旁边已填的数字冲掉了，加 paint-diff（`curValues / painted / paintedKeys`）只重画「上一次真画到 DOM 且被用户改过」的项；② **删主体不能顺手删模板** —— 会变成「还在但界面永远看不到」，detach 放进数据层（`entity-store.remove` 内调 `templates.detachEntity` 转回全局）而不是 UI 里，凡是「换个入口改这块数据」的路径，UI 层的调用者都会忘；③ **Pro 判定在测试里假红** —— 只 mock `EuriskoPlan.isPro` 不够，`apiClient.getCurrentUser()` 拿不到用户会走到本地 owner 分支把 ownerId 串到下一个用例，`beforeEach` 必须连 `window.apiClient` 一起删。守护测试 `tests/entity-template.test.js` 12 条（零感知 4 / 对拍 2 / 界面 2 / 额度 3 / 挂载 1）。不做：主体业务化、跨设备同步（要后端）、台账与批量（留给 19-10 / 19-11，也不在个人中心留占位卡 —— 占位卡点了没反应比没有更糟）。上一版 **v1.90.0**（门禁 **259/259**、单测 **100 套件 1900 例**、线上指纹 **37 项**）——「阶段19-8 效率层 E5：参数记忆 / 键盘 / 复制为表格 / 带参链接」：新增 `src/js/ui/param-memory.js` —— 进工具自动带出上次输入，三条边界是「只在**算得出来**时记」（记一份算不出结果的参数，下次带出来就是页面坏了）、「只记能原样还回去的值」（NaN 一律丢，还原出来是空框等于记忆记错了）、「带出必须说出来 + 给一个清空」（静默替换默认值，用户会以为是自己填的，最后把账算在算法头上）；优先级 = 调用方带进来的（历史查看 / 带参链接）> 上次输入 > spec 默认值；完整测算那份「草稿」记的是**没算完的半截 + 走到第几步**，两者分工不混（速算器一屏算完，没有半截这回事）。新增 `src/js/ui/param-link.js` —— `?t=<工具>&p=<参数>` 零后端把「这个工具 + 这组输入」变成一串 URL（发同事复核 / 存书签当月度模板 / 换设备继续），边界是未知工具不打开、只收注册表声明过的字段（链接是外部输入，不按字段表过滤等于开一个任意参数注入的口子）、不改地址栏。键盘可全程无鼠标：速算器回车跳结果、向导回车下一步、数字框 `inputmode="decimal"` 弹九宫格、**结果步不接管回车**（接到保存上，会让只想换行的用户把一条结果存进历史）。「复制为表格」输出 TSV 且金额是**裸数字** （带 ¥ 会被 Excel 认成文本，贴进去不能求和，那这个按钮就白做了）。常用置顶**不做** —— 工具页第一组「最近使用」已按使用顺序给出 6 个（19-3；阶段19-10a 起只此一处，首页那张同名卡已撤），再叠一层置顶就是两套排序。上一版 **v1.89.0**（门禁 **259/259**、单测 **99 套件 1881 例**、线上指纹 **37 项**）——「阶段19-7 视图密度（简明 / 完整）§3.9 ①②」：新增 `src/js/ui/mode-pref.js`（`window.EuriskoModePref`）：默认简明、游客可用、就地切换即时生效并记住偏好；命名只描述**内容密度**（不叫「新手模式」—— 那个叫法会让 80% 的人为自尊直接关掉，从而永远看不到自己需要的参数）；模式只影响 3 件事（字段可见性 / 结果展开深度 / 完整测算步骤），**绝不影响**计算口径与可用工具范围 —— 这是防止一个产品长成两个产品的护栏。registry 只增 `level` 字段（默认 basic），先给 6 个高频完整测算各标 1 个 advanced **步**（速算器一个都没标 —— 它们只有 2~5 个字段且全部必填，分级只会把必填项藏起来）；简明视图把 advanced 步**合并**成一步「补充参数（可选）」而不是删掉，advanced 字段收进「⚙ 更多参数（可选）」折叠块且**永远留在 DOM 里**（折叠 ≠ 删除，所以切换视图不丢已填的值）；入口是速算器页 / 完整测算页右上角那颗 pill（两处共用同一份 HTML，不写第二套措辞）+ 我的 → 账户设置 + 算满 3 次才问一次的轻提示。再上一版 **v1.88.0**（门禁 **259/259**、单测 **98 套件 1872 例**、线上指纹 **37 项**）——「阶段19-6b 我的页：权益进度条 + 资产概览 + 功能按场景分组（§3.6 ①②③）」：权益进度条 `#profile-benefits-card` 把"我是什么版本"换成"我用到了哪一步" —— 三格阶梯点亮到当前档，下面两行是真数（已用 N/M 套方案、云同步开没开 + 上次同步时间）；**进度条只表达档位在阶梯上的位置**，不表达"距离专业版还差百分之几"（基础版到专业版之间没有分母，任何百分比都是编出来的），取不到的整行不出现（不补 0 —— "已存 0 套" 会被读成"你一套都没存"），升级入口**未新增**（就是横幅那颗 `#profile-nav-upgrade` 搬了下来，全局仍只有 2 处）；资产概览四格全换 —— 原「计算次数 / 档案数量 / 历史记录 / 本月提醒」里两格**永远同一个数**（都读 `taxCalculationHistory.length`）、一格只有 0/1、一格只由当前月份决定，现为测算次数（本月 N 次）/ 已存方案 / 覆盖税种 / 上次测算，取不到一律「—」，最近一次按**时间**取最大（云同步合并后顺序不保证），「本月提醒」改挂税务日历卡的 badge；功能卡按场景分三组（我的数据 / 我的税务 / 服务与支持）。*对应项目版本：v1.78.0*
