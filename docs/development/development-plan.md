...
*文档创建时间：2026-05-25*
*最后更新：2026-09-15（**阶段15 全部关闭**：计划收口见 stage15-multi-tax-plan.md §9 —— 15A/15B/15C 交付 18 个落地页，15D 按事实标注（18 个 quick 模块 + 对拍测试、独立工具页入口、视觉统一实测验证 19 页共用 landing.css 零内联样式、门禁 251 项），15B-4 印花税并入 15B-3，15D-3 多税种热更新推迟至 16D B 端 API 一并参数化。15B-6 第十八个落地页 `/seo/disability-fund.html`「残保金与工会经费」（30 人免征、31 人按全部人数算、封顶社平 2 倍不是 300%、第一个残疾人最值钱）随 **v1.34.0** 交付；15B-5 第十七个落地页 `/seo/business-income.html`「个体工商户经营所得：核定 vs 查账」（核定不等于少交税、临界净利率 = 核定所得率 + 6 万 ÷ 年营收、减半减免额封顶 317250 元）随 **v1.33.0** 交付；15C 已于 v1.32.0 收尾（企业用工成本）。**阶段15 已开局并交付首批**：15C-2 第十五个落地页15C-3 第十六个落地页 `/seo/employer-cost.html`「企业用工成本」（成本倍数不是常数：工资越低倍数越高、涨薪传递率 50%~80%、只算法定五险一金）随 **v1.32.0** 交付；**阶段15 已开局并交付首批**：15C-2 第十五个落地页 `/seo/net-salary.html`「税后工资 / 谈薪倒算」（倒算不能除以到手率、涨薪看边际到手率、保底会咬人）随 **v1.30.0** 交付；15C-1 第十四个落地页 `/seo/social-base.html`「社保公积金」（缴费基数不是工资：60% 保底 / 300% 封顶，工伤生育个人不缴，公积金只有 12% 与社平 3 倍以内免税）随 **v1.29.0** 交付；15B-3 第十三个落地页 `/seo/surtax-stamp-duty.html`「附加税与印花税」（附加税跟着实际缴纳的增值税走、印花税按凭证税目且不含列明增值税、六税两费减半至 2027-12-31）随 **v1.28.0** 交付；15B-2 第十二个落地页 `/seo/corporate-income-tax.html`「企业所得税」（小微实际税负 5% 是「25% 计入 × 20% 税率」乘出来的、300 万·300 人·5000 万三门槛是临界点、分红再交 20% 个税）随 **v1.27.0** 交付；15B-1 第十一个落地页 `/seo/vat.html`「增值税」（企业税种开局：价外税先分离、小规模季 30 万免征且 3% 减按 1%、一般纳税人销项 − 进项留抵）随 **v1.26.0** 交付；15A-8 汇算页增强「多处任职 / 年中跳槽」（多段独立累计预扣 + 汇算合并、减除费用全年定额 6 万、把「档位重置」与「重复扣 6 万」两个补税机制分开）随 **v1.25.0** 交付；15A-7 第十个落地页 `/seo/early-retirement.html`（提前退休 / 内部退养一次性收入：真分摊 vs 平均只为定档，与离职补偿三套口径分开）随 **v1.24.0** 交付；15A-6 第九个落地页 `/seo/expat-allowance.html`（外籍个人津补贴免税：与专项附加扣除二选一 + 政策到期提醒接入注册表，2023 年第 29 号执行至 2027-12-31）随 **v1.23.0** 交付；15A-5 第八个落地页 `/seo/private-pension.html`（个人养老金：每年 12000 税前扣除 + 领取按 3% 单独计税 + 净优惠与回本线）随 **v1.22.0** 交付；15D-1 税种注册表 `tax-registry.js` + 15A-1 第四个 SEO 落地页 `/seo/labor-withholding.html`（劳务报酬 / 稿酬 / 特许权使用费预扣预缴）随 v1.18.0 交付 + 15A-2 第五个落地页 `/seo/equity-incentive.html`（股权激励个税）随 **v1.19.0** 交付 + 15A-3 第六个落地页 `/seo/severance.html`（离职补偿金个税：3 倍社平工资免税额度 + 单独计税，旧的平均法已不再执行）随 **v1.20.0** 交付 + 15A-4 第七个落地页 `/seo/special-deduction.html`（专项附加扣除：七项标准 + 年度确认 + 多抵多少试算）随 **v1.21.0** 交付；**阶段15 税务计算能力扩展（多税种）** 进行中（先 15A 个税纵深 → 后 15B 企业税种；纯前端能力，不依赖 ICP 备案、可与备案并行；收编阶段14 待办落地页「社保基数」「税后工资」），原「阶段15 迁移与合规升级」顺延为 **阶段16**（前置 ICP 备案；16D B 端 API 另依赖 15B 企业税种），方案见 `docs/development/stage15-multi-tax-plan.md` / `docs/development/stage16-migration-and-compliance-plan.md`；**阶段14 已全部完成：主体随 v1.13.0、剩余项「高商业意图 SEO 落地页」随 v1.14.0 / v1.15.0 / v1.16.0 陆续上线**：14A/14B/14C `ProCode` 专业版兑换码（后端事务化一码一用 + 用户端自助兑换 + 管理台「兑换码」Tab）与 **14D/C2 城市社保参数库**（`CitySocialConfig` + 公开只读端点 + 版本化回滚；顺带消除 C1 遗留的 `TODO(C2 社保地区政策库)`）已完成并随 v1.13.0 上线 —— 其中「参保城市」下拉已于 **v1.17.0 回退**（计算页回到「默认基数 + 用户自改」，城市改由留资/咨询时收集、顾问人工核对），参数库与端点保留待「社保基数」SEO 落地页复用；**阶段13 补强**：留资弹窗的「当前测算」由编造的入口标签改为可核实的非金额摘要（新增 `lead-context.js`），去掉无法兑现的「免费核对一次」次数承诺，并补上分享落地首屏引导（新增 `share-landing.js`）与分享图视觉收口；门禁 **251/251**、单测 **46 套件 928 例**、线上指纹 **37 项**（以上为 v1.34.0 当前基线）；**阶段14 剩余项已交付三页**：高商业意图 SEO 落地页 —— `/seo/bonus-tax.html`（年终奖个税）随 v1.14.0、`/seo/salary-tax.html`（月薪个税，累计预扣口径）随 v1.15.0、`/seo/annual-settlement.html`（汇算清缴：应退/应补 = 全年应纳税额 − 已预缴税额）随 v1.16.0 上线，均为静态正文 + 同源口径速算器（与内核逐点对拍）+ `robots.txt`/`sitemap.xml` 收录 + 门禁逐档对账断言，方案见 `docs/development/seo-landing-plan.md`；**待办**：其余关键词落地页（社保基数 / 税后工资）。上一版：阶段13 获客与转化 13A–13E 随 v1.12.0 上线（门禁 100/100、单测 20 套件 412 例）；阶段12 A+C1 已完成，支付体系与 B 端 API 因 ICP 备案阻塞移至阶段16。详见 CHANGELOG.md）*
*对应项目版本：v1.17.0*
