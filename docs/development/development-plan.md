...
| 阶段19：UI 重构与留存设计 | 🚧 **进行中**（**v1.77.0 起 · 2026-09-20**）：19-0 样式沙箱层 `src/css/ui-redesign.css` + **视觉回归截图基线**（`tools/ops/ui-screenshot-baseline.js`，6 页 × 375/1280 = 12 张，入仓）✅ v1.77.0；19-1a 设计令牌迁回 `tokens.css` 真源（零像素变化）✅ v1.78.0；19-2 Tailwind 产物**纯重建**（修好构建链路，零视觉变化）✅ v1.79.0；19-3 **全站阴影收口**到 `--sh-*` 5 个角色（26 处字面值收口、焦点环统一）✅ v1.80.0。**遗留**：`src/css/admin.css` 仍不可复现（`build:css` 不要整条跑）；深色档变化截图覆盖不到，深色模式需逐页人工过一遍。方案见 [stage19-ui-redesign-plan.md](./stage19-ui-redesign-plan.md) | 进行中 | 进行中 |
...
7. **UI 重构与留存设计（阶段19）** 🚧 进行中（v1.77.0 起 · 2026-09-20）
   - 已交付：19-0 样式沙箱层 + 视觉回归截图基线（12 张）/ 19-1a 令牌迁回真源 / 19-2 Tailwind 产物纯重建 / 19-3 全站阴影收口到 `--sh-*` 五角色
