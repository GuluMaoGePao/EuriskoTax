...
> 分支 `feature/10a-cloud-sync`：`--no-ff` 合入 main（`f33d694`）后随 v1.7.0 发布，已按分支规范删除本地与远程分支，历史留存于 tag `v1.7.0`（见文末决策记录）。
...
*文档创建：2026-09-07 · 2026-09-07 决策评审通过（D1/D2/D3 全部确认）；2026-09-08 10A 落地、2026-09-09 10B 落地后更新为 v1.0（阶段10 全部完成，待合入 main/发布；支付单列阶段11）*
...
| 2026-09-08 | 10A 执行中拆两里程碑：M1 后端地基（账户分层 + SEED_GRANT_PRO + sync 端点，commit a29bd12）→ M2 前端同步链路（plan.js + history-sync.js + 保存/删除挂钩 + plan 徽标/同步状态 UI） | 确认 |
| 2026-09-08 | 10A 验收：`verify:local` 35/35 全绿（新增前端静态断言 + A/B 双设备引擎沙箱 e2e），Jest 220/220；DoD 各项达成，分支待合入 main | 完成 |
| 2026-09-09 | 10B 范围落地（PDF 报告/政策推送，与 10A 一起发布）：服务端 `GET /api/content/tax-policy` 只读内容端点（仓库 JSON 源）；前端 `tax-policy.js` 专业版增量合并 + 抽屉「政策已更新」提示；`final-report.js` 专业版汇算清缴报告（品牌封面/税负对比图/政策要点/免责页，免费保留现预算表 PDF 无倒退） | 完成 |
| 2026-09-09 | 10B 验收：`verify:local` 42/42（+5 前端静态 + 2 内容端点 e2e）、Jest 242/242（+tax-policy/final-report 套件）；DoD 全部达成，待合入 main/发布 | 完成 |
| 2026-09-10 | v1.7.0 发布后补门禁覆盖（+10 项 → 52/52）：反馈附图（落库/非法 400/用户端与运维后台取回）+ 运维后台用户端点（401/列表搜索/详情计数/权益调档）。新增断言当场抓到 `GET /api/admin/users?q=` 在本地 SQLite 500（`mode: 'insensitive'` 不受支持），已按数据源协议修复 | 完成 |
| 2026-09-10 | `feature/10a-cloud-sync` 生命周期收口：`--no-ff` 合入 main（f33d694）→ 随 v1.7.0 发布 → 按分支规范（合完删除本地与远程分支）删除本地与远程分支。删除前已核验本地/远程两处 tip 均为 main 祖先（0 个独有提交），历史由 tag v1.7.0 承担 | 完成 |
| 2026-09-13 | 「版本与权益」权益口径按实现校正（阶段14 收尾时发现）：§3.1 与 §5.2 写的「政策更新推送」已自阶段11 起变为**全体用户（含游客）可见**，不再构成专业版差异，已从专业版权益移入基础版；「优先问题跟进与数据保障」当时未落地且至今无实现，已下线。专业版权益收敛为**真实生效的档位差异**：云端同步 · 多设备漫游、汇算清缴 PDF 完整报告、方案对比库 2 → 10 套（`scenario-store` 上限）、云端数据长期保留。§5.2 的实现描述本身保持不变（增量拉取仍在，只是不再限档） | 完成 |
