...
| `wechat` | 二选一 | 1–64 字符 |
| `company` | — | ≤ 100 字符 |
| `province` | — | 所在省份 / 直辖市，≤ 20 字符。**前后端均选填**（v1.39 起前台不再强制，此前为前台必填）；省份不能只是「筛选城市的中间态」—— 顾问要按省收敛分派（江浙沪私域），且市名重名时（吉林市 / 海南藏族自治州 vs 海南省）只有省 + 市组合才认得出统筹区 |
| `city` | — | 所在城市，≤ 20 字符。**前后端均选填**（v1.39 起前台不再强制，此前为前台必填；接口层一直只做长度约束）；用于顾问核对当地社保/公积金缴费基数口径 —— 计算页已不再让用户选参保城市（v1.17.0）。改选填的理由：城市对顾问有用，但不值得用它卡住提交（等于凭空丢弃 `lead_submit`）；与 `province` 各自独立校验、独立落库（用户可能只改城市而省份没重选） |
