...
describe('工具注册表：数量与分组', () => {
    ...
    test('20 个速算器 + 深度流程（0 个页面式 + 18 个 spec 驱动）', () => {
        ...
        //   有大病医疗的人，它一律算成「不补不退」。
        // 17D-8（v1.64.0）：special-deduction-deep 专项附加扣除（12/16）—— 第八个。速算器收的是
        //   **一个**「扣除前全年应纳税所得额」，于是「夫妻之间怎么分摊最省」「按实际符合条件的月份
        //   累计」「逐项边际节税」这三层它一个都答不了（实测分摊差 4656 元）。
        expect(specDriven).toEqual([
            ...
            'early-retirement-deep', 'equity-deep', 'expat-deep', 'forward', 'private-pension-deep',
    ...
    test('spec 驱动的完整测算复用速算器的字段与计算（不是复制）', () => {
        ...
        expect(twins).toHaveLength(14);

        // ① 共享速算器 spec 的**已经归零**（v1.62.0 收官）。17C 五条做完后，11 个有孪生
        //    速算器的 -deep 全部改成自带 spec —— 速算器只认「一个数」，而完整测算要算的
        //    恰恰是那几层口径：
        //      vat-deep（v1.58.0）：只认一个 `inputTax` 标量，「这笔进项能不能抵」表达不出来；
        //      corporate-income-tax-deep（v1.59.0）：三门槛看全年季度平均值、加计扣除整档掉到 5%、亏损过期；
        //      social-base-deep（v1.60.0）：把「税前月薪」当缴费基数，法定是本人上年度月平均工资；
        //      disability-fund-deep（v1.61.0）：把「在职职工人数」当年末在册，法定是上年月平均；
        ...
        const OWN_SPEC = ['annual-settlement-deep', 'bonus-tax-deep', 'corporate-income-tax-deep', 'disability-fund-deep',
            ...
            'social-base-deep', 'special-deduction-deep', 'private-pension-deep',
        ...
        // ② 自带 spec 的（17D 起）：速算器只认**一个数**，完整测算要算速算器算不了的那几层 ——
        //    共享同一份 fields 就等于把速算器复制一遍，那不是复用。它们不再共用对象，
        ...
        [
            ...
            { id: 'special-deduction-deep', twin: 'special-deduction', policy: 'special-deduction' },
            { id: 'private-pension-deep', twin: 'private-pension', policy: 'private-pension' }
        ].forEach((pair) => {
            const own = R().get(pair.id);
            expect(own.compute).not.toBe(R().get(pair.twin).compute);
            expect(own.fields).not.toBe(R().get(pair.twin).fields);
            expect(Array.isArray(own.fields) && own.fields.length > 0).toBe(true);
            expect(Array.isArray(own.steps) && own.steps.length > 0).toBe(true);
            expect(own.policyKey).toBe(pair.policy);   // 时效提醒仍指向同一条登记
        });
