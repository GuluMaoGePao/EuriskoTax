...
describe('备案内容合规（承诺书级）', () => {
    ...
    test('对外页面不做「该不该买 / 该不该参加 / 划不划算」这类投资或消费判断', () => {
        // 承诺书：不涉及投资理财业务。算税额是允许的，「值不值得」不是 ——
        // 个人养老金、企业年金、税优健康险三页曾写过「不划算 / 净收益 / 值得买 / 足以成为购买理由」，
        // 全部改成了税额事实描述（v1.37.9~v1.37.10）。改回来这里会红。
        const banned = ['不划算', '划算吗', '值得参加', '值得买', '值不值得', '净收益', '购买理由'];
        const hits = [];
        PUBLIC_PAGES.forEach((p) => {
            const src = read(p).replace(/<!--[\s\S]*?-->/g, ''); // HTML 注释不是页面内容
            banned.forEach((w) => {
                if (src.includes(w)) hits.push(p + ' → ' + w);
            });
        });
        expect(hits).toEqual([]);
    });
