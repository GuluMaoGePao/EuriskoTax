...
var surtaxRules = {
    ...
    // 阶段17 17C-4 纵深（v1.62.0）：计税依据是「**依法实际缴纳**」的增值税、消费税，
    // 但**退税分两种、方向相反**，且进口 / 代扣代缴的增值税**根本不附征** —— 这三处
    // 速算器都表达不出来（它只有一个「实际缴纳的增值税」输入框）。
    //   ① 增值税期末留抵退税额：**允许**从计税依据中扣除（财税〔2018〕80 号）；
    //   ② 即征即退 / 先征后返退还的增值税：**不扣**，已征的附加税也不退还；
    //   ③ 进口货物或者境外单位向境内销售劳务、服务、无形资产缴纳的增值税、消费税：
    //      **不征收**城建税及两项附加（城建税法第三条）。
    creditRefund: {
        label: '增值税期末留抵退税额',
        deductible: true,
        note: '实行增值税期末留抵退税的，允许从城建税及两项附加的计税依据中扣除退还的增值税税额'
    },
    instantRefund: {
        label: '即征即退 / 先征后返退还的增值税',
        deductible: false,
        note: '对增值税实行先征后返、即征即退办法的，随增值税附征的城建税及两项附加不予退还'
    },
    importVat: {
        label: '进口环节 / 代扣代缴的增值税',
        taxable: false,
        note: '进口货物或者境外单位和个人向境内销售劳务、服务、无形资产缴纳的增值税、消费税税额，不征收城市维护建设税'
    },
    // 小规模纳税人月销售额 10 万（季度 30 万）以下免征增值税 → 附加税跟着免（不是渐近，是整段跳变）
    smallThreshold: {
        monthly: 100000,
        quarterly: 300000,
        rate: 0.01,           // 超过门槛后按 1% 征收率（3% 减按 1%，至 2027-12-31）
        effectiveFrom: '2023-01-01',
        expiresOn: '2027-12-31'
    }
};
...
var stampDutyRules = {
    ...
    // 阶段17 17C-4 纵深（v1.62.0）：三处速算器表达不出来的口径
    //   ① 签订时**无法确定金额**的：先按 5 元贴花，以后结算时按实际金额计税、多退少补（第六条）；
    //   ② 在**境外书立、境内使用**的应税凭证，同样应当缴纳印花税（第一条）—— 不是「国外签的就不贴」；
    //   ③ 营业账簿只对**增加部分**计税：按实收资本（股本）+ 资本公积的**增加额**，不是每年按总额重贴。
    undeterminedPrepaid: 5,
    overseasNote: '在中华人民共和国境外书立在境内使用的应税凭证，应当依照本法缴纳印花税 —— 境外签订的合同不是「不用贴」',
    incrementNote: '营业账簿按实收资本（股本）、资本公积**合计金额的增加部分**计税，不是每年按总额重贴一遍',
    items: [
...
var socialInsuranceRules = {
    ...
    //
    // ① **缴费基数是「本人上年度月平均工资」，不是本月工资**。速算器的字段就叫
    //    「税前月薪」，compute 直接拿它当基数 —— 于是「月薪 1 万 + 年终奖 12 万」的人，
    //    基数被算成 1 万，而法定是 2 万。工资总额的口径见 wageComposition，
    //    且基数**一年一调**（多数地区每年 7 月随上年度社平工资公布调整），不是每月跟着工资变。
    // ② **公积金免税的两个上限是「且」的关系**：比例 ≤12% **且** 基数 ≤ 社平 3 倍。
    //    公积金基数可与社保基数不同（部分地区另行公布上下限），超出的部分并入工资计税。
    // ③ **申报基数不足额的代价**（社保费由税务部门征收后）：补缴 + 按日万分之五滞纳金
    //    （年化 18.25%）+ 欠缴数额 1~3 倍罚款。
    // ④ **灵活就业是另一套制度**：养老 20% 全部个人承担，其中 8% 进个人账户、12% 进统筹；
    //    基数在社平 60%~300% 之间**自选**（与单位职工「按实际工资定」根本不同），
    //    且**统筹部分不退还**。
    wageComposition: {
        // 国家统计局《关于工资总额组成的规定》第四条：工资总额由六个部分组成
        included: [
            { key: 'wage', label: '计时工资 / 计件工资' },
            { key: 'bonus', label: '奖金（年终奖、季度奖、绩效奖等）' },
            { key: 'allowance', label: '津贴和补贴' },
            { key: 'overtime', label: '加班加点工资' },
            { key: 'special', label: '特殊情况下支付的工资' }
        ],
        // 同一规定第十一条：不列入工资总额的范围（这些**不进**缴费基数）
        excluded: [
            '有关劳动保险和职工福利方面的费用',
            '劳动保护的各种支出',
            '出差伙食补助费、误餐补助',
            '对自带工具来企业工作的职工所支付的工具等的补偿费',
            '计划生育补贴、独生子女补贴',
            '实行租赁经营单位的承租人的风险性补偿收入',
            '支付给参加企业劳动的在校学生的补贴'
        ],
        note: '缴费基数是**本人上年度月平均工资**（上年度工资总额 ÷ 实际计薪月数），不是本月工资；新参加工作的职工从起薪之月起，上年度工作不满 12 个月的按实际月数平均',
        adjustNote: '基数**一年一调**（多数地区每年 7 月随上年度社平工资公布调整），不是每月跟着工资变'
    },
    flexible: {
        pensionRate: 0.20,          // 灵活就业人员参加企业职工基本养老保险：缴费比例 20%
        personalAccountRate: 0.08,  // 其中 8% 记入个人账户
        poolRate: 0.12,             // 12% 记入统筹基金
        medicalRate: 0.09,          // 职工医保：由统筹地区确定（多数按社平的一定比例），此处取常见值 9%
        levels: [0.6, 0.8, 1, 2, 3],// 可在当地社平工资 60%~300% 之间**自选**缴费基数
        note: '灵活就业人员在当地社平工资 60%~300% 之间**自选**缴费基数；养老保险按 20% 缴纳，全部由个人承担',
        refundNote: '**统筹部分不退还**：断缴、身故、出国定居只退还个人账户（8%）那一部分'
    },
    compliance: {
        lateFeeDailyRate: 0.0005,   // 《社会保险法》第八十六条：自欠缴之日起按日加收万分之五（年化 18.25%）
        penaltyMin: 1,              // 处欠缴数额 1 倍以上 3 倍以下罚款
        penaltyMax: 3,
        note: '用人单位未按时足额缴纳社会保险费的，责令限期缴纳或者补足，并自欠缴之日起按日加收万分之五的滞纳金；逾期仍不缴纳的，处欠缴数额一倍以上三倍以下的罚款',
        since: '2019-01-01'         // 社保费由税务部门统一征收（国税地税征管体制改革方案）
    },
...
var disabilityFundRules = {
    ...
    // 阶段17 17C-5 纵深（v1.61.0）：现行减缴是**阶段性**的，到期后按 100% 征收 ——
    // 做三年预算时不能只按 90%/50% 估，这个对照不是「可能」，是政策的明文到期日。
    tiersExpiryNote: '分档减缴与 30 人以下暂免均执行至 2027-12-31；到期后若无延续文件，应缴费额恢复按 100% 征收',
    smallExempt: {
    ...
    headcountNote: '在职职工人数 = **上年各月在职职工人数之和 ÷ 12**（不是年末在册）；含在编人员与签订 1 年以上（含）劳动合同的人员；季节性用工折算年平均人数；劳务派遣用工由派遣单位与用工单位协商计入一方（不得重复计算）'
};
