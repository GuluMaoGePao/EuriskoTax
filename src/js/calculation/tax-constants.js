...
// 社保/公积金缴费基数最低标准 —— 「全国平均」兜底值。
//
// 阶段14 C2 契约（运行时热更新，勿破坏）：
//   这两个值是分层生效的最终兜底，实际生效顺序为
//     tax-constants.js（本文件，全国平均兜底）
//       → tax-rates-sync.js 覆盖为管理台「税率」Tab 维护的全国口径
//       → city-social-sync.js 按用户所选参保城市覆盖为当地口径（最终生效值）
//   城市参数库由管理台「社保基数」Tab 维护（GET /api/config/city-social）。
