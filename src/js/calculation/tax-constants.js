...
// 税优健康险 —— 商业健康保险税前扣除（阶段15 15A-5 尾巴）
//
// 政策依据：《关于将商业健康保险个人所得税试点政策推广到全国范围实施的通知》
//           （财税〔2017〕39 号，自 2017-07-01 起全国实施）：
//   对个人购买符合规定的商业健康保险产品的支出，允许在当年（月）计算应纳税所得额时
//   予以税前扣除，扣除限额为 2400 元/年（200 元/月）；单位统一为员工购买符合规定的
//   商业健康保险产品的支出，分别计入员工个人工资薪金，视同个人购买。
//   保险赔款本身免征个人所得税（保险法/个税法第四条），所以这里**没有领取环节的税** ——
//   与个人养老金「领取按 3%」最大的差别就在这一步。
var healthInsuranceRules = {
    rateTable: 'comprehensiveTaxRates',   // 扣除降低的是年度综合所得的应纳税所得额（与内核同一张表）
    annualLimit: 2400,                    // 2400 元/年（200 元/月），超限部分当年不可扣
    monthlyLimit: 200,
    payoutTaxFree: true,                  // 保险赔款（理赔金）免征个人所得税，无领取环节税
    effectiveFrom: '2017-07-01',          // 财税〔2017〕39 号全国实施
    expiresOn: null                       // 长期政策，无到期日
};

// 企业年金 / 职业年金 —— ETE 递延纳税（阶段15 15A-5 尾巴）
//
// 政策依据：《关于企业年金 职业年金个人所得税有关问题的通知》（财税〔2013〕103 号，
//           自 2014-01-01 起施行）：
//   1. 缴费环节（E 暂不纳税）：
//      - 单位缴费部分：按季度/年度分配计入个人账户时，个人暂不缴纳个人所得税；
//        单位缴费不超过本企业职工工资总额 8% 的部分（单位+个人合计不超过 12%）
//      - 个人缴费部分：在不超过本人缴费工资计税基数的 4% 标准内的部分，暂从个人
//        当期应纳税所得额中扣除；计税基数为本人上年度月平均工资，超过当地上年度
//        职工月平均工资 300% 以上的部分**不计入**计税基数
//   2. 投资环节（T 暂不纳税）：年金基金投资运营收益分配计入个人账户时，暂不缴税
//   3. 领取环节（E 领取缴税）：个人达到法定退休年龄等领取年金时，**不并入综合所得**，
//      全额单独计算应纳税款 —— 按月领取的，按月度税率表（换算后）计算；
//      按季领取的，平均分摊到各月；按年领取的，按综合所得税率表计算
var annuityRules = {
    rateTable: 'comprehensiveTaxRates',   // 缴费环节少交的税按年度综合所得税率表算（与内核同一张表）
    monthlyRateTable: 'bonusMonthlyTaxRates',  // 按月领取时按月度税率表（与年终奖月度换算表同一张）
    personalRateCap: 0.04,                // 个人缴费 ≤ 计税基数 4% 部分当期从应纳税所得额中扣除
    employerRateCap: 0.08,                // 单位缴费 ≤ 工资总额 8%（单位+个人合计 ≤ 12%）
    combinedRateCap: 0.12,                // 单位 + 个人缴费合计上限 12%
    baseCapMultiplier: 3,                 // 计税基数封顶：当地上年度职工月平均工资的 300%
    investmentTaxFree: true,              // 投资环节收益暂不征税
    withdrawTaxOnFull: true,              // 领取环节按领取额全额单独计税（本金 + 收益）
    withdrawNotCombined: true,            // 领取不并入综合所得、不参与汇算
    effectiveFrom: '2014-01-01',          // 财税〔2013〕103 号
    expiresOn: null                       // 长期制度，无到期日
};
...
var corporateIncomeTaxRules = {
    ...
    // 阶段17 17C-2 纵深（v1.59.0）：三件速算器「收了数却没说口径」的事。
    //
    // ① 小微三条件里的**从业人数与资产总额，看的是全年季度平均值，不是期末数**
    //    （国家税务总局公告 2019 年第 2 号）：
    //      季度平均值 =（季初值 + 季末值）÷ 2；全年季度平均值 = 全年各季度平均值之和 ÷ 4
    //    所以 **12 月 31 日裁员到 300 人以下没有用** —— 速算器只收一个数，
    //    把「填哪个数」这件事推给了用户，而这正是最容易踩空的一处。
    // ② 研发费用加计扣除：不是「少交一点税」，而是**能把应纳税所得额压回 300 万门槛以内**，
    //    也就是从 25% 档掉回 5% 档 —— 边际收益在临界点是**跳变**的，不是线性的。
    // ③ 以前年度亏损**会过期作废**：一般企业 5 年，当年具备高新 / 科技型中小企业资格的延长至 10 年。
    //    速算器只收一个「可弥补亏损」数字，不问这笔亏损是哪一年、还在不在弥补期。
    quarterlyAverage: {
        formula: '季度平均值 =（季初值 + 季末值）÷ 2；全年季度平均值 = 全年各季度平均值之和 ÷ 4',
        note: '从业人数与资产总额均按**全年季度平均值**判定 —— 期末突击裁员、年末处置资产都不改变这个数',
        effectiveFrom: '2019-01-01'
    },
    rdSuperDeduction: {
        ratio: 1.00,              // 未形成无形资产计入当期损益的：据实扣除后再按 100% 加计扣除
        capitalizedRatio: 2.00,   // 形成无形资产的：按无形资产成本的 200% 在税前摊销
        advancedRatio: 1.20,      // 集成电路与工业母机企业：120%
        // 不适用加计扣除的负面清单行业（财税〔2015〕119号）
        excluded: [
            { key: 'tobacco', label: '烟草制造业' },
            { key: 'catering', label: '住宿和餐饮业' },
            { key: 'retail', label: '批发和零售业' },
            { key: 'realestate', label: '房地产业' },
            { key: 'leasing', label: '租赁和商务服务业' },
            { key: 'entertainment', label: '娱乐业' }
        ],
        note: '负面清单行业不得加计扣除；加计扣除直接减少应纳税所得额，够得着 300 万门槛时会**整档掉到 5%**'
    },
    lossCarryForward: {
        years: 5,                 // 一般企业：亏损结转年限 5 年
        extendedYears: 10,        // 当年具备高新 / 科技型中小企业资格的：延长至 10 年
        extendedFor: ['高新技术企业', '科技型中小企业'],
        note: '先到期的先弥补；超过结转年限仍未弥补完的**作废**，不再抵税',
        // 延长至 10 年的精确口径：具备资格年度**之前 5 个年度**发生的尚未弥补完的亏损
        extendedScope: '限于具备资格年度之前 5 个年度发生的、尚未弥补完的亏损'
    },
...
var vatRules = {
    ...
    // 阶段17 17C-1 纵深（v1.58.0）：进项税额的**两道闸门**。
    //
    // 速算器只收一个标量 `inputTax`「当期进项税额」，于是「这笔进项到底能不能抵」被推给了用户 ——
    // 而增值税最贵的一类错误就在这里。法定口径是**两道闸门都过才能抵**：
    //   ① **有没有合规扣税凭证** —— 普票、收据、白条一律不是扣税凭证（最常见的一笔「以为能抵」）；
    //   ② **用于什么用途** —— 免税项目、集体福利、餐饮娱乐、非正常损失一律不得抵扣，
    //      即使拿到的是专票也不行。
    // 还有第三类：**分不清用途的共同进项**（房租、水电、办公用品），按比例分摊转出 ——
    // 这是法定的第二十九条公式，速算器完全表达不出来。
    // 数字与文案同源：vat-deep 的 select options 直接读这里，一条都不复制。
    inputRules: {
        vouchers: [
            { key: 'special', label: '增值税专用发票', ok: true, note: '凭票抵扣，需在开票之日起 360 日内勾选确认' },
            { key: 'customs', label: '海关进口增值税专用缴款书', ok: true, note: '需先稽核比对相符后抵扣' },
            { key: 'vehicle', label: '机动车销售统一发票', ok: true, note: '扣税凭证是抵扣联' },
            { key: 'toll', label: '通行费增值税电子普通发票', ok: true, note: '左上角注明「通行费」字样；纸质通行费发票不可抵' },
            { key: 'normal', label: '增值税普通发票', ok: false, note: '普票不是扣税凭证 —— 最常见的一笔「以为能抵」' },
            { key: 'none', label: '收据 / 白条 / 未取得发票', ok: false, note: '无合规扣税凭证一律不得抵扣' }
        ],
        usages: [
            { key: 'business', label: '用于应税项目', ok: true, note: '凭证与用途均符合' },
            { key: 'exempt', label: '用于免税 / 简易计税项目', ok: false, note: '不得抵扣；已抵扣的要作进项税额转出' },
            { key: 'welfare', label: '用于集体福利 / 个人消费', ok: false, note: '职工食堂、宿舍、团建、发福利等购进的货物与服务' },
            { key: 'service', label: '餐饮 / 居民日常 / 娱乐 / 贷款服务', ok: false, note: '购进这四类服务一律不得抵扣，即使取得专票' },
            { key: 'loss', label: '非正常损失对应的进项', ok: false, note: '管理不善造成被盗、丢失、霉烂变质；依法没收、销毁、拆除' }
        ],
        // 无法划分用途的共同进项按比例分摊转出（财税〔2016〕36 号附件1 第二十九条）：
        //   不得抵扣的进项税额 = 当期无法划分的全部进项税额
        //                     × (当期简易计税项目销售额 + 免征增值税项目销售额) ÷ 当期全部销售额
        apportionNote: '房租、水电、办公用品这类**分不清用途**的共同进项，按免税与简易项目销售额占比分摊转出'
    },
    // 一般纳税人**不是想选简易就能选简易**：只有法定情形才可以，且**一经选择 36 个月内不得变更**。
    // 所以「一般计税 vs 简易计税」不是一道自由选择题 —— 先过法定情形这一关，再谈金额。
    simplifiedCases: {
        lockMonths: 36,
        items: [
            { key: 'none', label: '不符合法定情形（只能一般计税）', rate: 0 },
            { key: 'construction', label: '建筑服务：老项目 / 清包工 / 甲供工程', rate: 0.03 },
            { key: 'transport', label: '公共交通运输服务', rate: 0.03 },
            { key: 'realestate', label: '销售 / 出租 2016-04-30 前取得的不动产', rate: 0.05 },
            { key: 'labor', label: '劳务派遣服务（差额纳税）', rate: 0.05 },
            { key: 'culture', label: '电影放映 / 仓储 / 装卸搬运 / 收派 / 文化体育', rate: 0.03 },
            { key: 'goods', label: '特定货物：自来水、商品混凝土等', rate: 0.03 }
        ]
    },
...
// 残疾人就业保障金（残保金）与工会经费 —— 阶段15 15B-6
//
// 这两个都不是「税」，而是跟着**用工**走的规费，共同点是：**都按人 / 按工资总额算，
// 而且都和社保用的不是同一个口径** —— 这是这一页要钉住的主线：
//   · 残保金按「**差额人数** × **在职职工年平均工资**」算，不是按工资总额乘个比例；
//     招一个残疾人省下的是「一个人的年平均工资」，不是省 1.5% 的工资总额；
//   · 残保金用的年平均工资有**上限 = 当地社平工资 × 2**（2020 年起，此前是 3 倍），
//     注意社保的上限是社平的 **3 倍** —— 两个「封顶」倍数不同，不要混用；
//   · 工会经费按**工资总额** × 2%（工资总额含奖金津贴，无上下限），
//     不是社保缴费基数（有 60% 保底 / 300% 封顶），也不是税后工资。
//
// 数字来源与口径：
//   · 公式与安排比例：财税〔2015〕72 号第八条（未达规定比例的**差额人数** × 年平均工资）；
//     安排比例由各省在 **1.5%~1.7%** 内确定，页面默认 1.5%（《残疾人就业条例》规定不低于 1.5%）。
//   · 年平均工资上限：发改价格规〔2019〕2015 号自 **2020-01-01** 起，上限由社平 3 倍降为 **2 倍**。
//   · 30 人（含）以下企业**暂免**：财政部公告 2019 年第 98 号；分档减缴与免征均延续至 2027-12-31
//     （财政部 税务总局 中国残联公告 2023 年第 8 号）。到期日统一登记在注册表条目上，页面只读 statusOf。
//   · 工会经费：《工会法》（2021 年修正）第四十三条，按**全部职工工资总额 2%** 拨缴；
//     基层留存 60%、上缴上级工会 40%；企业所得税前扣除限额同为工资薪金总额 2%
//     （《企业所得税法实施条例》第四十一条），需凭《工会经费收入专用收据》等合规凭据。
var disabilityFundRules = {
    ratio: 0.015,             // 应安排残疾人就业比例（各省 1.5%~1.7%，页面默认 1.5%）
    ratioNote: '安排比例由各省、自治区、直辖市在 1.5%~1.7% 内确定，页面默认 1.5%，以当地规定为准',
    wageCapMultiple: 2,       // 年平均工资上限 = 当地社平工资 × 2（2020 年起；此前为 3 倍）
    wageCapNote: '残保金封顶是社平的 2 倍 —— 不是社保缴费基数那个 300% 上限，两个数不要混用',
    // 分档减缴：按「实际安排比例」分档（2023 年第 8 号公告，2023-01-01 至 2027-12-31）
    tiers: [
        { minRatio: 0.015, multiplier: 0, label: '达到规定比例（≥1.5%）：不缴' },
        { minRatio: 0.01, multiplier: 0.5, label: '1%（含）以上、未达规定比例：按 50% 征收' },
        { minRatio: 0, multiplier: 0.9, label: '1% 以下：按 90% 征收' }
    ],
    tiersEffectiveFrom: '2023-01-01',
    tiersExpiresOn: '2027-12-31',
    smallExempt: {
        headcountUpTo: 30,    // 在职职工人数 30 人（含）以下暂免
        effectiveFrom: '2020-01-01',
        expiresOn: '2027-12-31',
        note: '30 人是**临界点**：30 人免征，31 人就要按全部 31 人算（不是只对超出的 1 个人算）'
    },
    headcountNote: '在职职工人数含在编人员与签订 1 年以上（含）劳动合同的人员；季节性用工折算年平均人数；劳务派遣用工由派遣单位与用工单位协商计入一方（不得重复计算）'
};

// 工会经费（含未建会时的建会筹备金）：基数是**工资总额**，不是社保缴费基数
var unionFeeRules = {
    rate: 0.02,               // 按全部职工工资总额的 2% 拨缴（《工会法》第四十三条）
    remittedRatio: 0.4,       // 上缴上级工会
    retainedRatio: 0.6,       // 基层工会留存
    corporateDeductionRate: 0.02,  // 企业所得税前扣除限额：不超过工资薪金总额 2%
    wageBaseNote: '工资总额按国家统计局《关于工资总额组成的规定》口径：含计时/计件工资、奖金、津贴补贴、加班加点工资 —— 没有社保那样的 60% 保底与 300% 封顶',
    noUnionNote: '未建立工会的企业，由上级工会按工资总额 2% 收取建会筹备金（各地执行口径与起算时间不同，以当地工会与税务机关口径为准）',
