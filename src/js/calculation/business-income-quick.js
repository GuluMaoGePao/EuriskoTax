...
(function (global) {
    ...
    /**
     * 五级超额累进（**未减半**）—— 与内核 calculateBusinessTaxByTaxableIncome 同式。
     * 注意用的是经营所得自己的税率表（5%/10%/20%/30%/35%），不是工资那张七级表。
     */
    function taxBeforeHalveOf(taxable) {
        ...
        for (var i = 0; i < table.length; i++) {
            ...
            if (t <= b.max) {
                return {
                    tax: Math.max(0, t * b.rate - b.deduction),
                    rate: b.rate,
     ...
     * 与内核同一式子 —— 减免税额 = (min(应纳税所得额, 200 万) × 税率 − 速算扣除数) × 50%，
     * 再以「应纳税额不为负」兜底。
     */
    function halveOf(taxable) {
        ...
        if (t <= 0) return 0;
        var r = rules().halve;
        var before = taxBeforeHalveOf(t);
        var capped = Math.min(t, r.threshold);
        var cappedBracket = taxBeforeHalveOf(capped);
        // 上限内那部分对应的税额（用上限内金额单独走一次税率表，避免直接用全额档位放大减免额）
        var innerTax = Math.max(0, capped * cappedBracket.rate - cappedBracket.deduction);
        return Math.min(before.tax, innerTax * r.ratio);
    }
    ...
    function taxOf(taxable) {
        ...
        var before = taxBeforeHalveOf(t);
        var halve = halveOf(t);
        var tax = Math.max(0, before.tax - halve);
        return {
            taxable: t,
            ...
            effectiveRate: t > 0 ? tax / t : 0,
            halveApplied: halve > 0
        };
    }
