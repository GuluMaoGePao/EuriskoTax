...
## [1.106.0] - 2026-09-24 - 首页重排（P1）+ 工具页筛选（P2）+ 我的页三段式（P3）+ 备案位预留

### 病：首页 10 个区块按「功能有什么」堆，用户不知道先点哪个

P0 清掉了重复导航，但首页仍是 10 个区块纵向堆叠：身份是一种分类、日历是一种分类、历史是一种分类 ——
读起来「一堆都跟我有关」。同时三条「现在有个截止 / 有个没填」的事被拆成三张卡（我的税务资产 /
税务提醒 / 漏填提醒），只是三种分类视角，用户看到的是「三处都在催我」。

工具页的病是另一头：21 个完整测算 = 核心卖点 + 付费值厚度，却长在末位折叠组里 ——
默认收起等于对浏览者不存在。

我的页是第三头：13 张卡一维罗列，高频四项（历史 / 台账 / 档案 / 日历）与低频九项同权 ——
找「我的档案」要在一屏里翻。

### 改法

- **④ 接下来要办（三合一）**：原「我的税务资产」待办 + 原「税务提醒」+ 原「漏填提醒」收进同一张壳。
  三段各自的判定**不动**（各自的渲染函数仍管自己那段），新增 `syncTodoCard()` 只管
  「三段全空就整卡收起」—— 不重复实现任何一段的判定，避免长出第二套口径。
  截止段新增**90 天窗口**（`buildCalendarItems(now)`）：原先「经营所得减半优惠截止 2027/12/31」
  永远存在（剩 400+ 天），结果就是这张卡永远在、也永远没人看。判定抽成纯函数后，
  单测可以直接问「这天该出哪几条」，不必靠改系统时间或等某个月份。
- **① 搜索条常驻**：原「搜索入口卡」升级为顶部常驻条并上移到首屏最上（41 个入口早已越过
  「搜索优先于浏览」的 25 阈值）。仍只是工具页搜索的入口，不另起一套搜索实现。
- **⑤ 身份降 chip**：`我是谁` 整卡 → 一行 chip。身份是**设置默认视角**不是导航，
  因此 chip 形态不渲染描述与「看这类工具 ›」（那颗按钮正是它变成第二套分类的原因）。
  渲染分支挂在容器 class（`.identity-chip-row`）上，既有 fixture 仍是整卡，不受影响。
- **⑥ 继续上次 ≤3**：原「我的方案与台账」精简 —— 清单归「我的 → 计算历史」，首页只留「接着算」，
  最多 3 条（再多就是清单）。方案/台账段仍在卡内，但零感知：没内容就整段不出现。
- **工具页 T1 类型筛选**：`全部 41 / 速算 20 / 完整测算 21` 三个**互斥视图**（筛选 ≠ 混排，
  因此不违反「速算与完整测算不混排」的旧约束）。计数由注册表算，不写死数字；
  筛选状态是会话级的，**不写 localStorage** —— 下次进来停在子视图会被当成列表坏了。
- **P3 我的页三段式**：13 卡一维罗列 → 「头（身份 + 权益合一）· 高频四宫格 · 分组列表（9 卡）」。
  上提成宫格的四项是**计算历史 / 我的台账 / 税务档案 / 税务日历** —— 它们占「我的」真实点击的
  绝大部分，而原先散在「我的数据」「我的税务」两组里，找「我的档案」要往下翻一屏。
  宫格是**扫视**型（看图标定位），列表是**阅读**型（逐行读标题）：高频用前一种。
  入口定义仍只有 `PROFILE_CARDS_CONFIG` **一份**（多一个 `quick` 标记决定落在哪一段）——
  写两份的后果是同一入口两个落点，那正是去重铁律要拦的事，所以配了一条断言钉住
  「宫格与列表不得出现同一个 id，且两边加起来仍是全部 13 个入口」。
  被掏空的两组撤掉，「数据管理」归「工作台」（九卡 = 工作台 4 + 服务与支持 5）。
  §6.2 的 sub-nav「返回」**实测现状已符合**（`back-from-*` 统一走 `goBack`：回上一页历史、
  历史为空才回首页），本版不改 —— 计划里列的病，落地前要先确认它还在不在。
- **工具页 T2 / T3**：桌面分组头 sticky（网格化之后往下滚看不到「这一屏属于哪一组」）；
  桌面加一行 sticky 场景锚点 chip 条（横向跳读，不是左侧分类树）；只剩一组时锚点条不出。
  手机两条都不加：单列折叠滚两下的事，吸顶条只会吃掉一屏里最贵的 40px。
- **备案位预留（F-02 / F-03）**：ICP 还没下来，页脚位置**先占住**（原先空号是 `display:none`，
  号一到会多出一行、把底栏往上顶）。占位行只陈述主体名 + 状态，**绝不写形似备案号的假号** ——
  未备案展示备案号，性质是违规不是文案瑕疵。号下发后只改 `site-filing-ui.js` 里那两行
  （`icpNumber` / `policeNumber`），全站 22 个页面同生效；`tests/site-filing.test.js` 把
  「填号之后会长什么样」钉成断言，到那天改完配置跑测试，绿了就是对的。

### 验收

- 单测 **115 套件 2130 例**全绿（新增 `home-ia` 6 例 / `site-filing` 7 例 / 工具页筛选与锚点 6 例 /
  我的页宫格 3 例；`home-page` 的日历断言按 90 天窗口重写 —— 旧断言靠那条远期节点才通过，
  那条正是本次要清的噪音；`profile-page` 的「13 个模块卡片」同步改为 9 卡 + 宫格 4 项）。
- 门禁 `verify:local` **259/259**、线上指纹 **37 项**；`npm run verify:release` 五处版本号一致。
- **待办**：首页 / 工具页 / 我的页 / 顶栏视觉基线需重拍（28 张），三批改动已全部落地，
  可以一次性重拍了（不每小步重拍）。

---

## [1.105.0] - 2026-09-24 - 面向用户文案收敛：单一真源 + 去掉做不到的时效承诺

### 病：同一句免责声明散落 6 处、留资承诺了一个做不到的响应时效

两件具体的事：

1. **免责声明有 8 种写法**：结果区、助手回答、速算器报告页脚、Word 报告、分享图、21 个落地页
   各写一遍（后缀出现过「以税务部门核算为准」「以税务机关核定为准」「以主管税务机关认定为准」等
   8 种）。改一处漏一处的后果不是文案不统一，是**对外承诺的可信度口径不一致**。
2. **留资弹窗承诺「1 个工作日内联系您」**：顾问值班不可控，写进页面就是虚假承诺
   （广告法 + 个保法「告知不实」）。用户拍板：做不到就不写，统一「顾问会尽快与您联系」。

同期定位切换（不做付费 → 全部留资）也要求站内的「免费领取 / 了解专业版 / 线上购买通道」整体下线。

### 改法：单一真源 + 守护断言 + 批量脚本

- **`src/js/copy/copy-standard.js`（新增）**：面向用户文案的唯一出处。免责（A/B/C/D/E 五型）、
  隐私表述、留资诉求、升级码、档位 gate 提示、广告法黑名单全部收在这里；
  `src/js/**` 一律引用常量，不再字面量硬编码。它排在所有消费方脚本之前（index.html 装配断言覆盖）。
- **`tests/copy-standard.test.js`（新增，8 条守护断言）**：静态 HTML（index.html 与 seo/*.html）
  引用不了 JS 常量，那两处靠断言兜住 —— 免责主句不得改字、结果区两处文案不得漂移、
  极限词与付费营销词不得回潮、**留资时效词不得出现**、备案位一页不能漏、术语必须统一。
- **`tools/ops/seo-copy-batch.js`（新增）**：21 个落地页的术语 / 极限词 / 免责后缀 / 备案位批量对齐，
  支持 `--check`（CI 用）。改文案的正确顺序写进脚本头注释。
- **留资去掉全部时效**（§2.5 铁律）：弹窗信任三枚第三枚 → 「顾问核对后与您联系」；
  提交成功统一一句「已收到您的信息，顾问会尽快与您联系，请留意来电或微信。」；
  二维码区删掉「服务时间：工作日 9:00–18:00」。
- **升级码弹窗重做（PAY-04）**：原「基础版 vs 专业版」权益对比表与「线上购买通道后续开放」FAQ
  整体下线 → 只留「有码就开通 / 没码就留资」；14 天体验领取链路（PAY-01）与
  `handleClaimTrial` 一并删除（已领取用户权益到期前照常生效）。
- **术语统一**：税务部门 → 主管税务机关、客服 → 顾问、完整计算器 → 完整测算；
  描述性「最」（最容易填错 / 最值钱）改中性表述。
- **收尾两处口径**：落地页三处附加建议句统一为 C 型枚举（投资 / 保险产品购买 / 企业年金参加），
  公告弹窗统一称「公告与更新」。`verify:local` 里那条按旧措辞断言的项同步改字 ——
  文案只有一处真源，改文案就必须改断言，否则门禁会以「页面写错了」的形式报出来。
- **阶段20 P0 导航去重（R1 / R2 / R5）**：顶栏撤掉「个人中心」（它是目的地不是账号动作，
  与 Tab「我的」同名重复）与帮助按钮（使用帮助已有我的页卡 + 助手 FAB 两处）；
  首页底部「关于本站」长文删除，压缩成页尾一行信任标签 `41 个场景 · 数据不出本机`，
  完整口径（含本机计算承诺）移入「关于我们」弹窗。
  三处都是**删 DOM**，代价最高的失败模式是「删了 DOM 忘了删绑定」——
  `getElementById` 取到 null 后在 DOMContentLoaded 里抛错，后面所有初始化静默全残，
  所以 `tests/index-assembly.test.js` 新增三条断言同时钉两头：DOM 里没有 + 脚本里不许再引用。

### 验收

- 单测 **115 套件 2127 例**全绿（新增 copy-standard 8 例与导航去重 3 例；plan / profile-benefits /
  落地页脚本清单等 12 处旧断言按新文案改写）；门禁 `verify:local` **259/259**、线上指纹 **37 项**；
  `npm run verify:release` 五处版本号一致。
- `node tools/ops/seo-copy-batch.js --check` 为空操作。
- 视觉基线：升级码弹窗由「两栏权益对比」改为「单栏码输入 + 留资出口」，首页页尾与顶栏也变了，
  **需重拍首页 / 顶栏 / 升级码弹窗基线**（阶段20 P1 首页重排时一并重拍，不每小步重拍）。

---

## [1.104.0] - 2026-09-23 - 41 个工具真机冒烟（阶段19 收官验证）

### 病：单测全绿 ≠ 41 个入口在真机上出得来数

阶段19 一路踩的都是同一类坑 —— jsdom 全绿、真机点开一看不对（18-6a 行名带括号后缀精确匹配不到、
19-12 政策徽标把「月薪个税」压成一字一行、18-2 点一条老记录看到的是最近一次算的东西）。
单测钉的是「函数在给定输入下返回什么」，钉不住「41 个入口在真机上出不出数」：deep 侧尤其 ——
21 个结果卡由渲染器按 spec **现拼**，spec 缺字段 / compute 抛错 / 渲染少判空，
在单测里都是绿的那一格（测试只挑两三个代表跑）。

### 改法：真机上把 41 个工具各算一次（`npm run smoke:ui`）

`tools/ops/ui-smoke-all-tools.js`：20 个速算器 + 21 个完整测算逐个打开算一次，只问三件事 ——
出不出数、抛没抛异常（页面级 error 一并收，不只靠 try/catch）、deep 那枚时效徽标在不在。
三条边界：
- **不写留痕**：跑前拍 localStorage 快照、跑后**整份还原** —— 41 次测算灌进历史，
  历史就不再是「我算过什么」，后面拍基线也会被污染（首页历史卡与最近测算都在那儿）。
- **只报不修**：失败退出码 1 交给人判断 —— 自动化改代码 = 埋雷。
- **复用基线底座**（`ui-lib/screenshot-kit` 的服务器与浏览器调用），不另起一套；会话也复用
  `ui-baseline` —— 全新会话要拉 6 个外网 CDN（图标是字体），实测冷启动能卡 7 分钟以上。

### 顺带：阶段19 效率层的盘点结论（纠正两次误判）

按「先摸清再动手」查了一轮，结论是**效率层早已全部交付**，不是半成品：
模板（19-9）、台账（19-10）、批量（19-11）、参数记忆与草稿（19-8：速算器打开即带出上次输入、
deep 断点续算）、「与上次差 ¥X」（19-5b）、税务日历与首页待办（`home-mission` 的 `buildTodos`）、
首页台账卡（`home-ui` 的 `buildLedgerModel`，出口是 `openLedger` 真出口）。
批量也早就不是「只算工资表」—— 工具下拉已放开到全部 20 个速算器，每行都走同一个 `tool.compute`。
plan 里剩下的要么是**刻意不做**（常用置顶；待办 × 台账 —— 拿用户自己标的 status 去催人会变成瞎催），
要么**要后端**（主体跨设备同步、登录后随档案同步）。这两类都不该由前端单方面拍板，
所以本版不做新功能，改做**收官验证**。

### 验收

- `npm run smoke:ui`：**41/41 出数**、零页面级异常；唯一「有政策版本但无时效徽标」的是
  `classification` —— 19-12 的已知缺口（政策库里没有分类所得条目，刻意不挂 `policyKey`）。
  这个缺口每次冒烟都列出来：悄悄多出几个，就说明有人新增 spec 时忘了挂。
- 单测 **113 套件 2104 例**全绿、门禁 `verify:local` **259/259**、线上指纹 **37 项**；
  本版只加脚本、文档与 npm script，**不动产品代码，视觉基线无需重拍**。

---

## [1.103.0] - 2026-09-23 - 政策时效与版本钉上结果页（19-12）

### 病：`tax-registry.js` 里的时效 API 睡了大半年觉

`statusOf` / `expiringWithin` / `effectiveFrom` / `expiresOn` **全都早就有**（30 条政策条目，
每条带生效与到期日），UI 上却只在速算器页渲染了一枚徽标 —— 21 个完整测算
（财务与代账的主力入口，他们最怕的就是「政策过期没」）一枚都没有。
同一个 policyKey，速算器页看得到有效期、完整测算页什么都看不到：
这不是「少一个功能」，是**同一个产品在两个入口上给了两套可信度口径**。

### 改法：两类入口、同一枚徽标、外加一个版本号

- **徽标只有一份**：速算器出（`policyBadgeOf`），deep 只管用 —— 21 个完整测算的结果卡顶部
  现在钉着与速算器**同一枚**（已过期 →「结果仅供参考」；180 天内 →「优惠至 X · 剩 N 天」；
  长期 →「长期有效」）。它是**可信标记**不是参数，所以简明视图下照样显示 ——
  藏起来等于让用户不知道这次算里有没有已经过期的政策。
- **B4 顺手补上**：结果与 PDF 导出都标「政策版本 2026.3」（`tax-registry.VERSION`）。
  报告是要**交给别人看**的（老板 / 客户 / 税局），「按哪年政策算的」是它能不能被当证据用的前提。
  版本号渲染层**不自带数字** —— 自带就一定会在某次改口径时忘了同步，
  印着一个对不上的版本号比不标更糟，因为它看起来像证据。
- **测试抓出真数据洞**：6 个完整测算根本没挂 `policyKey`
  （forward / business / reverse / property-transfer / non-resident / classification）——
  没挂**不会报错**，只是那一页永远没有徽标，典型的静默缺口。补上 5 个能确定对应的；
  **classification（分类所得）刻意不挂**：政策库里没有这个条目，
  为凑齐徽标编一条政策（文号与有效期会印进导出的「政策依据」）比不挂更糟 ——
  不挂只是这一页没有时效徽标，挂错是给用户一个**错的有效期**，那是在骗人。
  缺口写进测试白名单（不许默默变多；补了真条目就删白名单，测试会逼着把 policyKey 挂上）。
- **顺手修一个基线抓到的真回归**：徽标挤在标题行里，手机宽度（375px）下把「月薪个税」
  压成一字一行 —— 时效与版本改为**独占一行**（可信标记值得占一行，标题不值得为它竖排）。

### 验收

- 新增 `tests/policy-freshness.test.js` **14 条**：21 个 spec 的 policyKey 无洞
  （缺口只许是显式白名单）/ 挂的 key 都真在政策库里（挂了个不存在的 id 同样静默不出）/
  deep 结果卡里的徽标**就是速算器那一枚**（同一函数出的同一段 HTML，不是另写一份）/
  简明视图下也显示 / 三种时效措辞 / registry 拿不到就静默不出
  （不确定就说不确定，绝不装作长期有效）/ 版本来自 registry、不给就不标、也不编一个。
- 视觉基线重拍 4 个 target ×2 宽度 ×2 主题（quick-375/1280、deep-result-375/1280，
  总量 28 张不变）：deep-result 上「政策有效期至 2027-12-31 · 政策版本 2026.3」
  钉在金额上方 —— 这正是 B3 + B4 要的样子。
- 单测 **112 套件 2092 例**全绿；门禁 `verify:local` **259/259**、线上指纹 **37 项**
  （本版未改门禁与指纹口径）。
- 真机已验：deep 结果卡与速算器页的徽标文本**逐字一致**（「长期有效 政策版本 2026.3」）。

---

## [1.102.0] - 2026-09-23 - 视图信号写进线索：谁真的想要细节，顾问该先看谁（19-7b ②）

### 病：两条天差地别的线索，在线索表里长得一模一样

一条是「完整视图 + 调过三个进阶参数」的财务，一条是「简明视图 + 只填了月薪」的路人 ——
系统里存的都是姓名 + 电话 + 一句情境。可用户早就在界面上交代了自己是谁：**要不要看更多参数，
是一次明确的自我介绍**。这信息以前白扔了，而且它是零成本的：不用多问一句，
不用多采一项个人信息（不含任何金额与填报内容，只记「使用深度」）。

### 改法：一条链路、四个落点，信号不许被稀释

1. **记录放在 mode-pref.js**（折叠块是它定义的：简明 = 收起）。两个渲染器各自发明一个存储键，
   就是阶段18 那个病 —— 加一种形态漏一批，最后谁也说不清「调过进阶参数」到底记没记。
2. **只有简明视图下的展开才计**：完整视图下折叠块本来就是展开的，点一下不代表在找参数；
   算进去会把「完整视图用户」和「简明视图下摸索过的人」混成同一个数，分层直接失效。
   同理**只有「更多参数」这个块算**，推导链那些 `details` 不算。
3. **监听写在 document 的捕获阶段**：`toggle` 事件**不冒泡**，而速算器与向导都是整块 innerHTML
   重画，逐个 details 绑定会被下次重渲染冲掉（非冒泡事件仍走捕获阶段，这是唯一能一劳永逸的挂法）。
4. **必须落库才有用**：顾问看的是线索表与导出的 Excel，信号只存在浏览器里等于没做 —— 线索表
   加 `view_mode` / `advanced_touched` 两列 + 迁移 + CSV 两列。两条口径：
   `view_mode` **允许为空**（老客户端没上报就是没信号，不能回落成 `simple` 充数，
   否则顾问会把「没采集到」当成「用户选了简明」）；24 小时合并时 `advanced_touched` 取 **max**
   —— 它是事实不是状态，不能被后一次「这次没展开」抹掉，抹掉等于把已知的强信号降级。

刻意**不并进 `current()` 的情境文本**：那段话是给用户看的「参考您的测算」，
混进行为标记，就成了当着用户的面给他打分。取不到信号（模块没加载）返回 `null`：
没信号就说没信号。

### 验收

- 新增 `tests/lead-view-signal.test.js` **14 条**：简明下展开计数并记下是哪个测算 /
  **完整视图下不计数** / 合上不算 / 反复展开累加而同一测算只归因一次 /
  别的折叠块（推导链）不算 / 折叠块带 `data-tool-id`（否则只知道有人调过参数，不知道调的哪个税）/
  `reset` 清空 / `viewSignals` 汇总 / 偏好模块不在时返回 `null` / 不污染 `current()` 文本 /
  **跨文件字段名对拍**（payload 字段名 = 后端读取名 = schema 列名 = CSV 列序）。
  `tests/leads.test.js` 加 **2 条**：`viewMode` 非法值回落空串、`advancedTouched` 只认非负整数
  （它是计数，脏数据回落 0 只会**低估**信号，不会凭空造出一个高优线索）。
- 跨文件字段名之所以要单独钉：这条链路跨 4 个文件，任一侧改名都**不报错**，
  只是顾问从此看不到信号 —— 静默失效里最难发现的那一类。
- 单测 **111 套件 2078 例**全绿；门禁 `verify:local` **259/259**、线上指纹 **37 项**
  （本版未改门禁与指纹口径）。
- 真机已验（ui-baseline 会话，走的是真实路径不是 API 拼装）：简明下展开 → `count=1`、
  `tools=['annual-settlement-deep']`；切到完整视图再展开 → **不计数**；
  **拦下 `apiClient.submitLead` 的入参，payload 里 `viewMode` / `advancedTouched` 真的带上了**
  （这条最值钱：jsdom 测不到 payload 组装，只有真机能证明字段真的上了车）。
- 视觉基线 **28 张零差异**：本版只加了一个 `data-tool-id` 属性与一个存储键，没有任何可见变化
  —— 这正是它该有的样子（信号采集不该改界面）。

---

## [1.101.0] - 2026-09-23 - 字段级分级铺到全部完整测算（19-7b ①）

### 病：简明视图只在一小半工具上真的简明

19-7a（v1.89.0）给 6 个高频完整测算各标了一个「步」级 `advanced`，简明视图会把那一步
合并成「补充参数（可选）」。**其余 15 个 spec 一个分级都没有** —— 默认简明的人点进去，
看到的还是一整屏参数，双视图对它们等于没上线。

### 改法：判定标准只有一条，声明收在一张清单里

一个字段标 `advanced` 当且仅当：**不填也能算出结果**（有 `default` 兜底）、填了更准。
据此三条硬边界：

- **主输入不标**（月工资 / 收入总额 / 成本 / 各类金额）—— 藏起来不叫简洁，叫算不出来；
- **决定「这次算什么」的分支不标**（身份 / 情形 / 征收方式 / 是否并入 / 是否享受优惠）——
  藏了等于悄悄换掉工具语义，用户会以为自己算的是另一件事；
- **整步都是可选的 → 标步级**（简明下合并成一步），**步内只剩一部分可选 → 标字段级**
  （收进「⚙ 更多参数（可选）」折叠块）。两者**不叠加**：既在 advanced 步里又自带 level
  会折出两层折叠，用户要连点两次才看得到一个参数。

分级写在注册表末尾的 `ADVANCED_FIELDS` / `ADVANCED_STEPS` 两张清单里，**不散到 300 个字段
定义上逐个打 `level`**：一份清单能一眼看全「哪些参数算进阶」，散进字段定义就再没人核对得过来
—— 这与「按名字认人的表，每加一种形态就漏一批」是同一个病，同一剂药（按 id 认，不按位置认）。
速算器一个都不标（沿用 19-7a 口径）：它们只有 2~5 个字段且全部必填。

### 顺手修：一个字段写了两个 `step`，把自己从界面上抹掉了

`private-pension-deep` 的 `growthMultiple` 原先是
`{ step: 'pension', …, step: 0.1 }`（末尾那个 0.1 想当输入框步长）。同一对象里后者覆盖前者，
字段的 `step` 变成 `0.1` —— 不属于任何一步，界面上**永远看不到它**，只能按 `default=1.5`
参与计算：能算、但改不了。渲染器不读字段级步长（数字框统一 `inputmode=decimal`），
所以那个 `0.1` 直接删。测试里加了一条「每个字段的 step 都能在 spec.steps 里找到」，
这类错以后会直接红。

### 验收

- 新增 `tests/field-level.test.js` **9 条**：分级铺到了全部 spec（`classification` 只有 1 个
  字段，是唯一允许 0 分级的）/ **标了 advanced 的字段必须有 default**（「不填也能算」是分级的
  前提，收起来没值等于换了一套算法）/ 每个 spec 至少留一个 basic 字段（简明视图不该是空表）/
  **步级与字段级不叠加** / 字段的归属步必须存在 / 速算器一个都没标 /
  简明下 advanced 控件仍在 DOM 里且默认收起、完整下默认展开（折叠 ≠ 删除）。
- 视觉基线补一张 target `deep-step1-advanced`（年度汇算简明视图第 1 步，×2 宽度 ×2 主题共 4 张，
  基线 24 → 28 张）：19-7b 唯一肉眼可见的产物原先**一张基线都没进过** ——
  deep-step1 / deep-result 用的 bonus-tax-deep 是 19-7a 就有的步级分级，
  新铺的折叠块没有回归网（改坏了哪种跑法都发现不了）。
- 单测 **110 套件 2062 例**全绿；门禁 `verify:local` **259/259**、线上指纹 **37 项**
  （本版未改门禁与指纹口径）。
- 真机已验（ui-baseline 会话）：简明下折叠块默认收起且正是劳务 / 稿酬 / 特许权三项、
  完整下默认展开、往折叠块里的字段填值后简明 ↔ 完整来回切**值不丢**、
  修好的 `growthMultiple` 在缴费年数旁可见。

---

## [1.100.0] - 2026-09-22 - 省钱卡接进 21 个完整测算（清偿 19-4 顺延：省钱卡）

### 病：速算器有省钱卡，按年填全的那些没有

19-6a（v1.87.0）的省钱卡只挂在速算器结果页。21 个完整测算的结果步算完就到免责声明 ——
而**真正会漏填专项附加扣除的恰恰是这些按年填全的测算**（一整年的收入、扣除、分摊都在里面，
漏一项就是一年）。plan §3.5 列的两个确定性场景里，「专项附加扣除漏填」这一半从没在 deep 侧出现过。

### 改法：适用范围由字段表说了算，不由工具名单说了算

原先 `deductionMissTip` 的第一句是 `if (tool.group !== 'salary') return null;` ——
速算器才有 `group`，21 个 deep spec 一个都没有，照这个判定它们永远进不来。
改成**按 spec 自己的 fields 判**：有 label 含「专项附加扣除」的字段 = 这次测算吃它，才可能漏。

- 这么判顺带**修正了一处既有误导**：`bonus-tax`（速算器）原先因为 `group === 'salary'`，
  档案里勾了扣除项时会被报「档案里有扣除，这次没算进去」—— 可**年终奖单独计税根本不吃
  专项附加扣除**，那张卡是在催用户去补一个对他这笔奖金无效的数。用字段表判它自然被排除。
- `special-deduction`（核定额度那个）同样自动排除：它的字段是「子女教育（个数）」这类逐项，
  没有一个叫「专项附加扣除」的字段 —— 它自己就是在算这个，不该提醒自己。
- 不写 id 白名单：加一个 spec 就要回来改一次名单，那是 19-5b 那条债的同一个形状
  （两份真源迟早漂移）。

### 出口：分步向导里是「回去补上」，不是「跳去另一个工具」

速算器那颗 CTA 是打开 `special-deduction` 核定额度 —— 在 deep 里照抄就是让用户把已经
分步填完的一整份丢掉重来。所以 tip 上带**两个**出口，由渲染层按「此刻真能用」挑：

- 能定位到那一步 → 「回去补上」（回到那一步改这次的测算，走的是与「上一步」同一套
  `state.stepIndex + saveDraft + render`，不为这张卡新开导航路径）；
- 定位不到（简明视图把 advanced 步合并进「补充参数」了）→ 退回「核定能扣多少」；
- 速算器没有 opts → 老路（聚焦输入框 / 打开核定工具），行为不变。

一颗按钮只做一件事，且点了必须真的发生点什么 —— 摆一颗点了不跳的按钮是拿 UI 骗人。

### 不做的事（写在这里，免得后人按 plan 字面 repeater）

plan §3.5 给省钱卡列了两个场景，第二个是**年终奖择优**。**deep 侧刻意不做**：

- `bonus-tax-deep` 本身就是在做择优，再告一遍「已为你选优」是噪音；
- 它算的是**含最优拆分**的口径，与速算器那份「两种计税方式差额」不是同一个数，
  拿过来用就是把两个口径混着说（这正是 19-5b 清掉的那类错）。

所以完整测算侧的省钱卡只有「专项附加扣除漏填」一条 —— 一条真的，好过两条里有一条是编的。

### 验收

- 新增 `tests/saving-nudge-deep.test.js` **11 条**：deep 结果步出卡 / 不编金额（卡里一个 ¥ 都没有）/
  这次带上就不出 / 档案没勾就不出 / 年终奖择优不重复提示 / 核定额度那个不提醒自己 /
  **速算器年终奖不再被误报**（钉住上面那处修正）/ 吃扣除的速算器照旧出卡（判定放宽没把老场景弄丢）/
  CTA 回那一步（点了真的回到输入步）/ 那一步找不到就退回核定工具且 `onStep` 不被调用 /
  无 opts 时走速算器老路。
- ⚠️ **真机未验**（与 v1.98.0 / v1.99.0 同一晚，playwright 会话仍起不来）。建议手动点：
  进「综合所得」完整测算 → 在扣除那一步关掉「享受专项附加扣除」→ 一路点到底 →
  结果步出现「还能少缴 · 档案里有扣除」卡（**没有 ¥ 金额**）→ 点「回去补上」回到那一步。
- 单测 **109 套件 2053 例**全绿（jest 实跑 2065 例）；门禁 `verify:local` **259/259**、
  线上指纹 **37 项**（本版未改门禁与指纹口径）。

---

## [1.99.0] - 2026-09-22 - 结果页「各项数额」条（清偿 19-4 顺延：结果可视化）

### 先纠一处 plan 的事实错误：Chart.js 从来就不在依赖里

plan §3.5 写「引入 Chart.js 轻量图表（**已在依赖里**，`final-report.js` 已用）」。
动手前查了一遍：全仓 **0 处引用**，`final-report.js` 也没用它 —— 这句话是当初写计划时想当然。
为一个横条引一个 canvas 库的代价是：新增 CDN 依赖、深色模式要重画（`tokens.css` 不会自动跟）、
24 张截图基线全部重拍、jsdom 里画不出东西因而测不到。所以**纯 CSS 横条**（`div` + `width%`），
颜色走既有令牌，深色 / 响应式 / 降级全部自动跟随，且能被单测钉住。

### 第二处偏离：不做四段瀑布，做「各项数额」

瀑布（收入构成 → 扣除 → 应税 → 税额）要靠**行名认语义**：哪一段是减项、哪一段是基数。
41 个工具（20 速算器 + 21 完整测算）没法通用给出，硬配一份映射就是每个工具一份 —— 这正
是 19-5b 那条债的由来（一份映射迟早与界面上正在显示的那个数漂移，v1.98.0 清的就是它）。
所以这张条**不猜语义**：只画各项金额的相对大小（最长一项 = 100%），主结果用品牌色标出来，
其余中性灰。说明里明写「各项之间不构成加减关系」—— 不让人把它当成构成图去求和。

### 三条纪律（与 v1.98.0 方案库同一套）

- **画不出就不画**：凑不出两根（只有主结果 / 结果为空 / 明细里没有金额行）整块不出现 ——
  一根条除了占地方什么也没说。
- **负数与 0 不上条，不拿 0 充一根条**：条没有负长度，取绝对值就是造一个不存在的数；
  **唯一例外是主结果** —— 它是这一页的主角，「这个季度不用交税」本身就是结论，
  把它滤掉等于把结论藏起来（实测：增值税小规模默认入参下主结果正是 0，小微免征）。
- **只认金额行**：把「实际税负率 13%」和「销项税额 ¥3,000」画在同一根尺子上比大小，
  是在制造一个不存在的比较。

### 落点

- 新增 `src/js/ui/result-bars.js`（纯函数 `buildBars` + `html`），样式在 `ui-redesign.css` §4.5。
- **一份实现、两处挂载**：速算器结果区（`toolbox-ui.js`）与 21 个完整测算的结果步
  （`deep-wizard-ui.js`）共用 —— 两边的结果对象本来就是 `primary + rows` 同一形状，
  写第二套必然漂移。取的都是**界面上正在显示的那一份**（`out` / `view`），不会再算一遍。
- 位置：主结果 Hero 之后、明细行之前（夹在结论与明细中间，不插到推导链后面去）。
- 最多 5 根，超出的在说明里写「只列金额最大的 5 项」—— **不静默截断**。
- 默认展开、可收起：折叠的本意是别占地方，默认收起则是把信息拿走。
- 模块没加载时 `console.warn` 明说，不静默吞掉（前车之鉴：`auth-ui.js` 死选择器靠 `?.` 抹错多年未发现）。

### 验收

- 新增 `tests/result-bars.test.js` **20 条**：画不出就不画 3 / 负数与 0 不上条且主结果为例外 6 /
  排序与刻度 5（主结果固定第一 · 最长一项 100% · 极小项留 2% · 同名只留一根 · 截断要说清）/
  渲染 6（两档颜色 · 说明写清不是加减 · 默认展开 · 行名转义 · 用传进来的格式化 · 无 toolbox 也有兜底）。
- `tests/toolbox-ui.test.js` 与 `tests/deep-wizard.test.js` 各加一条：钉住它**挂对了地方**
  （条在 `tool-result-rows` / `data-dw-row` 之前）且取的是同一份结果视图。
- ⚠️ **真机未验**：与 v1.98.0 同一晚，playwright-cli 会话仍起不来（`open` 无响应）。
  建议手动点一遍：任一速算器算完看结果区那张「各项数额」条 → 收起/展开 → 进一个完整测算
  走到结果步看同一张条 → 深色模式再看一次（条的底色与填充色要分得开）。
- ⚠️ **截图基线待重拍**：`tools/ops/ui-lib/screenshot-kit.js` 的 `TARGETS` 里 `deep-result`
  拍的是结果步、`quick` 打开即算（速算器一屏即算，默认入参就有结果）—— 两张图里都会出现
  这张新条，`--check` 必然对不上。等浏览器能起来时补跑
  `node tools/ops/ui-screenshot-baseline.js`（深浅各一次）并把产物入仓。
- 单测 **108 套件 2042 例**全绿（jest 实跑 2054 例）；门禁 `verify:local` **259/259**、
  线上指纹 **37 项**（本版未改门禁与指纹口径）。
