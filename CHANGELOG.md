...
- **本地开发测试账号更换**：开发环境用的测试账号由 `dev@example.com` / `password` 换为 `2649719969@qq.com` / `[REDACTED]`（用户名 `devuser` 不变）。改动面：`server/scripts/reset-dev-user.js`（账号定义处）+ `server/scripts/verify-local-auth.js`（门禁登录用的 `DEV_EMAIL` / `DEV_PASSWORD` 常量与断言文案）+ 登录页「开发环境：填入本地测试账号」预填 + `server/src/routes/auth.js` 的 Swagger 示例 + GUI 控制台的账号 / 密码 / 一键复制按钮、环境检查与取 Token 的登录体 + `tools/ops` 的 `get-token.ps1` / `debug-swagger*.ps1` 与公网分享邮件模板 + 相关文档（README / 开发工作流 / 开发账号 / GUI 按钮速查 / 看门狗文档）。**只影响本地与测试环境**：`reset-dev-user.js` 不随部署执行，生产库不会创建此账号；本地 `dev.db` 里既有的 `dev@example.com` 行**不自动删除**（该账号名下可能已有线索 / 计算记录，误删会连带丢数据），需要时在 GUI 数据库面板手动清理。顺带修正 `tools/gui/README.md` 里重置账号按钮的脚本路径（原写 `tools/ops/reset-dev-user.js`，实际为 `server/scripts/reset-dev-user.js`）。**门禁断言随之修正**：`GET /profile 身份校验` 原先拿用户名比对，但账号早已存在时用户名是用户自设昵称（脚本只在新建时才写 `devuser`），会把「账号已存在」误判成链路故障 —— 改为按邮箱（账号标识）比对，昵称仅作输出展示
