...
- ⚠️ **真机这一轮没跑成**：agent-browser 在本轮整体挂住（`open` 不返回、换会话名与换端口都一样，
  当天早些时候同一套脚本是好的），对比度审计与截图都卡在第一页。**因此 profile 页的截图基线仍是 v1.93.0 的**
  （工作台组多了一张「批量计算」卡），**下一轮浏览器恢复后要先补拍 profile 深浅两档**，
  批量页的样式与排版也待那时真机点一遍 —— jsdom 只盖到接线，盖不到排版。
  新增样式的对比度改为手工核算：`.batch-note` 取既有信息色令牌，浅色 6.95:1、深色 7.02:1 均过 AA；
  新卡片图标换 rose 档（amber-600 压 amber-100 只有 2.86:1，图标会糊在底色里，rose 3.91:1）。
