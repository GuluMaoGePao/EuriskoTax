            ...
            if ($proc) {
                ...
                if ($proc.ExitCode -eq 2 -and $name -like "verify-pg*") {
                    ...
                    if (Test-AllowPopup -Key "PG_NO_DOCKER") {
                        $pgMsg = "本机没有可用的 Docker，PostgreSQL 演练门禁已跳过（返回码 2）。`r`n`r`n这不是代码问题：`r`n  ① 日常发布前继续跑「✅ 本地登录链路验证（verify:local）」，SQLite 下同一套 220 项照样全跑；`r`n  ② 只有改了 server/prisma/schema.prisma 或 migrations/ 时才必须跑 PG 演练；`r`n  ③ 演练前请先停掉本地 :3000 后端（运行中会锁 Prisma 引擎 DLL，generate 会报 EPERM）。`r`n`r`n是否现在打开 Docker Desktop 下载页？"
...
Add-SectionCard -TabCtx $tab3Ctx `
    -Title "3. 发布门禁（上线前必跑）" `
    -Subtitle "脚本：server/scripts/verify-local-auth.js（同一套 220 项断言，两种数据库模式）" `
    ...
    @{ Text = "🐘 PostgreSQL 演练门禁`n（verify:pg · 与线上同序）"; Desc = "生产等价演练（约 2-4 分钟）：Docker 起临时 PostgreSQL（端口 55432）→ prisma generate → migrate deploy → 内容种子 → 同一套 220 项断言。需 Docker Desktop 已启动；未安装时会提示「跳过」并返回码 2（不是代码问题）。本地 :3000 后端运行中会锁 Prisma 引擎 DLL —— 点按钮时若检测到后端在跑会先警告二次确认。"; Color = "75, 140, 230"; Width = $BTN_WIDE_W;
    ...
    @{ Text = "🔄 全新库演练`n（verify:pg:fresh · 删卷重来）"; Desc = "等价「线上全新库首次部署」：先删演练数据卷（docker compose down -v）再跑一遍 migrate deploy + 220 项断言，验证从零建表的部署路径。同样需要 Docker Desktop（与上一个按钮共用 :3000 后端前置检查）。"; Color = "165, 105, 210"; Width = $BTN_WIDE_W;
