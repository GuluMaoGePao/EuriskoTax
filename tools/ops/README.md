...
| `ops-verify-pg.ps1` | **生产等价演练门禁**：Docker 起临时 PostgreSQL → `migrate deploy` → 内容种子 → 同一套 182 项 e2e（入口 `npm run verify:pg`）；无 Docker 时优雅退出（码 2） | ✅ |
...
```powershell
# 起临时 PostgreSQL → prisma generate → migrate deploy → 内容种子 → 182 项 e2e
npm run verify:pg
